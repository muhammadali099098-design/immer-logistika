...
app.post("/api/shipments", requireAuth, async (req, res) => {
  try {
    ...
    for (const l of lines) {
      ...
      const mk = `${o}__${Number(l.model_id)}`;
      sums.set(mk, (sums.get(mk) || 0) + Number(l.qty));
    }
    for (const [mk, qty] of sums) {
      const [o, mid] = mk.split("__");
      const avail = await availableQty(logistics_id, Number(mid), undefined, o);
      if (qty > avail) return res.status(400).json({ ok: false, error: `Нельзя отгрузить больше остатка (Order ${o}, модель ${mid}): доступно ${avail}` });
    }
...
app.put("/api/shipments/:id", requireAuth, async (req, res) => {
  try {
    ...
    for (const [mk, qty] of sums) {
      const [o, mid] = mk.split("__");
      const avail = await availableQty(Number(s.logistics_id), Number(mid), id, o);
      if (qty > avail) return res.status(400).json({ ok: false, error: `Нельзя отгрузить больше остатка (Order ${o}, модель ${mid}): доступно ${avail}` });
    }
    const cost = parseCbm(req.body.cost_amount);
...
const patch = {};
    ...
    if (req.body.receipt_status !== undefined) {
      ...
      if (req.body.receipt_status === "received") {
        const file = String(req.body.file_name || "");
        if (!file) return res.status(400).json({ ok: false, error: "Чтобы отметить прибытие, загрузите документ" });
        patch.receipt_file = file;
        patch.arrival_date = req.body.arrival_date || new Date().toISOString().slice(0, 10);
      }
    }
