...
app.post("/api/shipments", requireAuth, async (req, res) => {
  try {
    ...
    for (const l of lines) {
      if (!l.order_no) return res.status(400).json({ ok: false, error: "Выберите Order № для каждой позиции" });
      const avail = await availableQty(logistics_id, Number(l.model_id), undefined, String(l.order_no));
      if (Number(l.qty) > avail) return res.status(400).json({ ok: false, error: `У модели ${l.model_id} (Order ${l.order_no}) нельзя отгрузить больше, чем на складе (доступно ${avail})` });
    }
...
app.put("/api/shipments/:id", requireAuth, async (req, res) => {
  try {
    ...
    for (const l of lines) {
      if (!l.order_no) return res.status(400).json({ ok: false, error: "Выберите Order № для каждой позиции" });
      const avail = await availableQty(Number(s.logistics_id), Number(l.model_id), id, String(l.order_no));
      if (Number(l.qty) > avail) return res.status(400).json({ ok: false, error: `У модели ${l.model_id} (Order ${l.order_no}) нельзя отгрузить больше, чем на складе (доступно ${avail})` });
    }
    const cost = Number(req.body.cost_amount);
