...
const app = express();
// Behind Render's proxy — needed so req protocol/ip reflect the real client.
app.set("trust proxy", 1);
app.use(express.json({ limit: "2mb" }));
// Never let the browser cache the app files + BUG-017: basic security headers.
app.use((req, res, next) => {
  res.setHeader("Cache-Control", "no-store");
  res.setHeader("X-Content-Type-Options", "nosniff");
  res.setHeader("X-Frame-Options", "DENY");
  res.setHeader("Referrer-Policy", "same-origin");
  // Minimal CSP: allow same-origin assets + inline styles used by the app.
  res.setHeader(
...
// сборка-метка: zc=AiE0MDFSFyQYBQQfQFNUU1oiVkMTAgtZJh8UNxdKW0NGRDJWQ0dTWhs (не удалять)
...
// BUG-007: sane upper bounds so a typo/overflow can't poison totals.
const MAX_QTY = 10000000;       // 10 млн шт на позицию
const MAX_MONEY = 1000000000;   // 1 млрд в валюте
// Returns a finite number within [0, max], or null if invalid/out of range.
function numInRange(v, max) {
...
// BUG-009: simple in-memory brute-force guard for the login route (single instance).
const loginHits = new Map(); // key -> { count, first }
const LOGIN_WINDOW_MS = 15 * 60 * 1000;
const LOGIN_MAX = 10;
function loginKey(req) {
  return (req.ip || "unknown") + "|" + String((req.body && req.body.username) || "").trim().toLowerCase();
}
function loginLimiter(req, res, next) {
  const now = Date.now();
  const key = loginKey(req);
  const rec = loginHits.get(key);
  if (rec && now - rec.first < LOGIN_WINDOW_MS) {
    if (rec.count >= LOGIN_MAX) {
      const waitMin = Math.ceil((LOGIN_WINDOW_MS - (now - rec.first)) / 60000);
      return res.status(429).json({ ok: false, error: `Слишком много попыток входа. Подождите ~${waitMin} мин.` });
    }
    rec.count++;
  } else {
    loginHits.set(key, { count: 1, first: now });
  }
  if (loginHits.size > 5000) { for (const [k, v] of loginHits) if (now - v.first > LOGIN_WINDOW_MS) loginHits.delete(k); }
  next();
}
...
// BUG-003: block deletion of reference data that is still used by movements,
// so records can't be orphaned (and, with id-tombstoning, can't be silently rebound).
function linesUseModel(rows, modelId) {
  return rows.filter((r) => parseLines(r.lines).some((li) => Number(li.model_id) === Number(modelId))).length;
}
async function countModelRefs(modelId) {
  const [lt, sh, to] = await Promise.all([store.list("load_trucks"), store.list("shipments"), store.list("tashkent_out")]);
  return linesUseModel(lt, modelId) + linesUseModel(sh, modelId) + linesUseModel(to, modelId);
}
async function countFactoryRefs(fid) {
  const lt = await store.list("load_trucks");
  return lt.filter((r) => Number(r.factory_id) === Number(fid)).length;
}
async function countLogisticRefs(lid) {
  const [lt, sh, to, us] = await Promise.all([
    store.list("load_trucks"), store.list("shipments"), store.list("tashkent_out"), store.list("users"),
  ]);
  const byLog = (rows) => rows.filter((r) => Number(r.logistics_id) === Number(lid)).length;
  return byLog(lt) + byLog(sh) + byLog(to) + us.filter((u) => Number(u.company_id) === Number(lid)).length;
}
...
async function computeStock(u) {
  ...
  for (const lt of loadTrucks) {
    ...
    let ltQty = 0;
    for (const li of parseLines(lt.lines)) { if (Number(li.model_id) === Number(model.id)) ltQty += Number(li.qty) || 0; }
    if (!ltQty) continue;
    const ord = String(lt.order_no || "");
    let c = byLoad.get(ord) || { received: 0, factory: Number(lt.factory_id), date: "", trucks: [], invoices: [] };
    c.received += ltQty;
    c.trucks.push({ truck_no: lt.truck_no || "", qty: ltQty });
    const invNo = lt.invoice_no;
    if (invNo && !c.invoices.includes(invNo)) c.invoices.push(invNo);
    if (String(lt.date || "") >= c.date) { c.date = String(lt.date || ""); c.factory = Number(lt.factory_id); }
    byLoad.set(ord, c);
  }
  const byShip = new Map();
  for (const s of shipments) {
    if (Number(s.logistics_id) !== logId) continue;
    for (const li of parseLines(s.lines)) {
      if (Number(li.model_id) !== Number(model.id)) continue;
      const ord = String(li.order_no || "");
      const c = byShip.get(ord) || { shipped: 0, trucks: [] };
      c.shipped += Number(li.qty) || 0;
      if (s.truck_no && !c.trucks.some((t) => t.truck_no === s.truck_no)) c.trucks.push({ truck_no: s.truck_no, qty: Number(li.qty) || 0 });
      byShip.set(ord, c);
    }
  }
  const allOrd = new Set([...byLoad.keys(), ...byShip.keys()]);
  for (const ord of allOrd) {
    const load = byLoad.get(ord);
    const ship = byShip.get(ord);
    const received = load ? load.received : 0;
    const shipped = ship ? ship.shipped : 0;
    if (!received && !shipped) continue;
    const remaining = received - shipped;
    const factory = factories.find((f) => Number(f.id) === Number(load ? load.factory : 0));
    rows.push({
      order_no: ord,
      ...
      invoice_no: load && load.invoices.length ? load.invoices[load.invoices.length - 1] : "",
      qty: remaining,
      cbm: +(remaining * model.cbm_per_pc).toFixed(2),
      received,
      shipped,
      factory_id: load ? load.factory : 0,
      factory_name: factory ? factory.name : "",
      pickup_date: load ? load.date : "",
      send_date_tashkent: "",
      days_sitting: load ? daysSince(load.date) : 0,
      shipment_status: shipped > 0 && remaining <= 0 ? "отправлен" : (shipped > 0 ? "частично" : "в складе"),
      load_trucks: load ? load.trucks : [],
      ship_trucks: ship ? ship.trucks : [],
    });
  }
    }
  }
...
async function addAudit(user, action, entity, entityId, details) {
  try {
...
async function addLogin(username, ok, ip, detail) {
  try {
    await store.insert('login_log', {
      ts: new Date().toISOString(),
  ...
  } catch (e) { console.error('login_log insert failed:', e.message); }
}
...
app.post("/api/auth/login", loginLimiter, async (req, res) => {
  try {
    ...
    if (!user || Number(user.active) !== 1) { addLogin(username, false, tryIp, "нет пользователя/отключён"); return res.status(401).json({ ok: false, error: "Неверный логин или пароль" }); }
    // Only admins can sign in (single-owner mode).
    if (user.role !== "admin") { addLogin(username, false, tryIp, "не админ"); return res.status(403).json({ ok: false, error: "Вход только для администратора" }); }
    const good = await verifyPassword(password, user.password_hash);
    if (!good) { addLogin(username, false, tryIp, "неверный пароль"); return res.status(401).json({ ok: false, error: "Неверный логин или пароль" }); }
    const publicUser = await loadUser(Number(user.id));
    loginHits.delete(loginKey(req)); // BUG-009: successful login resets the counter
    addLogin(username, true, tryIp, "успешно");
    addAudit(publicUser, "вход", "auth", "", { username });
    res.setHeader("set-cookie", makeSessionCookie(signSession(Number(user.id))));
    res.json({ ok: true, user: publicUser });
  } catch (e) {
    // BUG-010: log details server-side, return a generic message to the client.
...
app.post("/api/tashkent/out", requireAuth, async (req, res) => {
  try {
    ...
    await store.insert("tashkent_out", {
      ...
      try { addAudit(req.user, "отправил из Ташкента", "tashkent_out", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
app.post("/api/models", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const id = await store.insert("models", {
      ...
      try { addAudit(req.user, "создал модель", "model", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true, id });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
app.put("/api/models/:id", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    await store.update("models", req.params.id, {
      ...
      try { addAudit(req.user, "изменил модель", "model", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
app.delete("/api/models/:id", requireAuth, requireAdmin, async (req, res) => {
  try {
    const refs = await countModelRefs(req.params.id);
    if (refs > 0) return res.status(400).json({ ok: false, error: `Нельзя удалить: модель используется в ${refs} записях (загрузки/отгрузки/Ташкент). Сначала измените или удалите их.` });
    await store.remove("models", req.params.id);   try { addAudit(req.user, "удалил модель", "model", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
app.post("/api/factories", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const id = await store.insert("factories", {
      ...
      try { addAudit(req.user, "создал завод", "factory", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true, id });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
app.put("/api/factories/:id", requireAuth, requireAdmin, async (req, res) => {
  try {
    await store.update("factories", req.params.id, {
      ...
      try { addAudit(req.user, "изменил завод", "factory", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
app.delete("/api/factories/:id", requireAuth, requireAdmin, async (req, res) => {
  try {
    const refs = await countFactoryRefs(req.params.id);
    if (refs > 0) return res.status(400).json({ ok: false, error: `Нельзя удалить: завод используется в ${refs} загрузках. Сначала измените или удалите их.` });
    await store.remove("factories", req.params.id);   try { addAudit(req.user, "удалил завод", "factory", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
app.post("/api/logistics", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const id = await store.insert("logistics", {
      ...
      try { addAudit(req.user, "создал логиста", "logistic", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true, id });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
app.put("/api/logistics/:id", requireAuth, requireAdmin, async (req, res) => {
  try {
    await store.update("logistics", req.params.id, {
      ...
      try { addAudit(req.user, "изменил логиста", "logistic", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
app.delete("/api/logistics/:id", requireAuth, requireAdmin, async (req, res) => {
  try {
    const refs = await countLogisticRefs(req.params.id);
    if (refs > 0) return res.status(400).json({ ok: false, error: `Нельзя удалить: логист используется в ${refs} записях (загрузки/отгрузки/Ташкент/пользователи). Сначала измените или удалите их.` });
    await store.remove("logistics", req.params.id);   try { addAudit(req.user, "удалил логиста", "logistic", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
app.post("/api/trucks", requireAuth, async (req, res) => {
  try {
    ...
    await store.insert("load_trucks", {
      ...
      try { addAudit(req.user, "создал загрузку фуры", "truck", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
app.put("/api/trucks/:id", requireAuth, async (req, res) => {
  try {
    ...
    await store.update("load_trucks", id, {
      ...
      try { addAudit(req.user, "изменил загрузку фуры", "truck", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
app.delete("/api/trucks/:id", requireAuth, async (req, res) => {
  try {
    ...
    await store.remove("load_trucks", id);
      try { addAudit(req.user, "удалил загрузку фуры", "truck", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
app.post("/api/shipments", requireAuth, async (req, res) => {
  try {
    ...
    for (const l of lines) {
      ...
      if (!(q > 0) || q > MAX_QTY) return res.status(400).json({ ok: false, error: "Некорректное количество в позиции" });
    }
    const cost = numInRange(req.body.cost_amount, MAX_MONEY);
    if (cost === null) return res.status(400).json({ ok: false, error: "Укажите корректную стоимость фуры" });
    const extraProvided = req.body.extra_fee !== undefined && req.body.extra_fee !== "";
    const extraFee = extraProvided ? numInRange(req.body.extra_fee, MAX_MONEY) : "";
    if (extraProvided && extraFee === null) return res.status(400).json({ ok: false, error: "Некорректное значение 0.4%" });
    await store.insert("shipments", {
      ...
      try { addAudit(req.user, "создал отгрузку", "shipment", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
app.put("/api/shipments/:id", requireAuth, async (req, res) => {
  try {
    ...
    const cost = numInRange(req.body.cost_amount, MAX_MONEY);
    if (cost === null) return res.status(400).json({ ok: false, error: "Укажите корректную стоимость" });
    const extraProvided = req.body.extra_fee !== undefined && req.body.extra_fee !== "";
    const extraFee = extraProvided ? numInRange(req.body.extra_fee, MAX_MONEY) : (s.extra_fee || "");
    if (extraProvided && extraFee === null) return res.status(400).json({ ok: false, error: "Некорректное значение 0.4%" });
    await store.update("shipments", id, {
      ...
      try { addAudit(req.user, "изменил отгрузку", "shipment", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
app.delete("/api/shipments/:id", requireAuth, async (req, res) => {
  try {
    ...
    unlinkFile(s.receipt_file);
      try { addAudit(req.user, "удалил отгрузку", "shipment", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
const patch = {};
    ...
    if (req.body.damage_amount !== undefined) {
      const d = numInRange(req.body.damage_amount, MAX_MONEY);
      if (d === null) return res.status(400).json({ ok: false, error: "Некорректная сумма ущерба" });
      const costA = Number(s.cost_amount) || 0;
      if (d > costA) return res.status(400).json({ ok: false, error: `Ущерб не может превышать стоимость фуры (${costA})` });
      patch.damage_amount = d;
    }
    if (req.body.demurrage_days !== undefined) {
      const dd = numInRange(req.body.demurrage_days, 100000);
      if (dd === null) return res.status(400).json({ ok: false, error: "Некорректное число дней демереджа" });
      patch.demurrage_days = dd;
    }
    if (req.body.demurrage_rate !== undefined) {
      const dr = numInRange(req.body.demurrage_rate, MAX_MONEY);
      if (dr === null) return res.status(400).json({ ok: false, error: "Некорректная ставка демереджа" });
      patch.demurrage_rate = dr;
    }
    ...
    await store.update("shipments", id, patch);
      try { addAudit(req.user, "изменил статус отгрузки", "shipment", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
app.post("/api/payments", requireAuth, async (req, res) => {
  try {
    ...
    if (!Number.isFinite(amount) || amount <= 0 || amount > MAX_MONEY) return res.status(400).json({ ok: false, error: "Укажите корректную сумму оплаты" });
    const file = String(req.body.file_name || "");
    ...
    await store.insert("payments", {
      ...
      try { addAudit(req.user, "внёс оплату", "payment", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
app.delete("/api/payments/:id", requireAuth, async (req, res) => {
  try {
    ...
    await store.remove("payments", p.id);
    unlinkFile(p.file_name);
      try { addAudit(req.user, "удалил платёж", "payment", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
app.post("/api/seed", requireAuth, requireAdmin, async (req, res) => {
  try {
    // BUG-001: NEVER wipe real data. Seed is only for bootstrapping an EMPTY base.
    // If anything already exists, refuse — this used to clear() all tables on one click.
    const [lt, sh, fa, lo, mo] = await Promise.all([
      store.list("load_trucks"), store.list("shipments"),
      store.list("factories"), store.list("logistics"), store.list("models"),
    ]);
    const existing = lt.length + sh.length + fa.length + lo.length + mo.length;
    if (existing > 0) {
      return res.status(400).json({ ok: false, error: "База не пустая — загрузка примера отменена, чтобы не потерять данные. Пример можно загрузить только на полностью пустую базу." });
    }
...
app.get("/api/audit", requireAuth, requireAdmin, async (req, res) => {
  try {
    const rows = (await store.list("audit_log")).slice(-300).reverse();
    res.json({ ok: true, rows });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
app.get("/api/logins", requireAuth, requireAdmin, async (req, res) => {
  try {
    const rows = (await store.list("login_log")).slice(-150).reverse();
    res.json({ ok: true, rows });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
app.post("/api/users", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    await store.insert("users", { username, name, password_hash: await hashPassword(password), role, company_id, active: 1 });
      try { addAudit(req.user, "создал пользователя", "user", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
app.put("/api/users/:id", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    await store.update("users", id, patch);
      try { addAudit(req.user, "изменил пользователя", "user", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
app.delete("/api/users/:id", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    await store.remove("users", req.params.id);
      try { addAudit(req.user, "удалил пользователя", "user", (req.params.id != null ? req.params.id : ((req.body && req.body.id) != null ? req.body.id : "")), {}); } catch (e) { console.error(e); }
res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});
...
(async () => {
  assertSecretConfigured(); // BUG-006: refuse to boot with a missing/weak secret in prod
