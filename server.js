  ...
  for (const model of models) {
    for (const log of logistics) {
      ...
      for (const lt of loadTrucks) {
        ...
        const cur = inv.get(key) || { qty: 0, factory: Number(lt.factory_id), date: "", trucks: [] };
        cur.qty += qty;
      ...
      const invArr = [...inv.entries()]
        .map(([invoice, v]) => ({ invoice, received: v.qty, remaining: v.qty, factory: v.factory, date: v.date, trucks: v.trucks }))
...
// Finance: per logistics, shipments with debt, due dates (arrival + 30d), payments.
app.get("/api/finance", requireAuth, async (req, res) => {
  try {
    ...
    for (const log of logistics) {
      if (scope != null && Number(log.id) !== Number(scope)) continue;
      const lShip = shipments.filter((s) => Number(s.logistics_id) === Number(log.id));
      if (lShip.length === 0) continue;
      const rows = lShip.map((s) => {
        const total = calcShipmentTotal(s);
        const arrival = s.arrival_date || s.date || "";
        const due = arrival ? addDays(arrival, 30) : "";
        let daysLeft = null;
        if (due) daysLeft = Math.ceil((new Date(due).getTime() - Date.now()) / 86400000);
        return { id: Number(s.id), truck_no: s.truck_no || "", total, arrival, due, days_left: daysLeft, paid: 0, debt: total };
      }).sort((a, b) => (a.due || "").localeCompare(b.due || ""));
      // Allocate the logistics' total payments FIFO by due date; leftover subtracts from the last truck.
      const pays = payments.filter((p) => Number(p.logistics_id) === Number(log.id)).sort((a, b) => String(a.date || "").localeCompare(String(b.date || "")));
      let pool = pays.reduce((a, p) => a + (Number(p.amount) || 0), 0);
      for (const r of rows) {
        if (pool <= 0) break;
        const take = Math.min(r.debt, pool);
        r.paid += take;
        r.debt -= take;
        pool -= take;
      }
      if (pool > 0 && rows.length) rows[rows.length - 1].debt -= pool;
      const rowsWithPay = rows.map((r) => ({ ...r, payments: pays.map((p) => ({ amount: Number(p.amount) || 0, currency: p.currency || "USD", file_name: p.file_name || "", date: p.date || "" })) }));
      const totalDebt = rows.reduce((a, r) => a + r.debt, 0);
      groups.push({ logistics_id: Number(log.id), logistics_name: log.name, rows: rowsWithPay, total_debt: totalDebt, total_paid: pays.reduce((a, p) => a + (Number(p.amount) || 0), 0) });
    }
    res.json({ ok: true, groups });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});

// Add a payment for a logistics (reduces debt FIFO across trucks). Requires an uploaded file.
app.post("/api/payments", requireAuth, async (req, res) => {
  try {
    ...
    await store.insert("payments", {
      logistics_id, amount, currency: req.body.currency || "USD",
