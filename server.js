...
async function computeStock(u) {
  ...
  // Остаток по модели у логиста меньше 5 шт. — автоматически списывается (qty → 0):
  // позиция уходит в «Отправленные», а разница (например, −4) показывается в «Граница → Ташкент».
  const remByKey = {};
  rows.forEach((r) => { const k = (r.logistics_id || 0) + "|" + (r.model_id || 0); remByKey[k] = (remByKey[k] || 0) + (Number(r.qty) || 0); });
  rows.forEach((r) => {
    const k = (r.logistics_id || 0) + "|" + (r.model_id || 0);
    const total = remByKey[k] || 0;
    if (total > 0 && total <= 5 && Number(r.qty) > 0) {
      r.swept = Number(r.qty);
      r.qty = 0;
      r.cbm = 0;
      r.days_sitting = 0;
      r.shipment_status = "отправлен";
    }
  });
