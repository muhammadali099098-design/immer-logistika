  ...
  try {
    ...
    for (const log of logistics) {
      ...
      const rows = lShip.map((s) => {
        ...
        const pays = payments.filter((p) => Number(p.shipment_id) === Number(s.id));
        ...
        return {
          id: Number(s.id), truck_no: s.truck_no || "", total, paid, debt, arrival, due, days_left: daysLeft,
      ...
      groups.push({ logistics_id: Number(log.id), logistics_name: log.name, rows, total_debt: totalDebt });
    }
    res.json({ ok: true, groups });
  } catch (e) { console.error(e); res.status(500).json({ ok: false, error: "Ошибка сервера" }); }
});

// Add a payment for a shipment (reduces debt). Requires an uploaded file.
app.post("/api/shipments/:id/pay", requireAuth, async (req, res) => {
  try {
    const id = Number(req.params.id);
    const all = await store.list("shipments");
    const s = all.find((x) => Number(x.id) === id);
    ...
    await store.insert("payments", {
      shipment_id: id, amount, currency: req.body.currency || "USD",
