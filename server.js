  ...
  for (const model of models) {
    for (const log of logistics) {
      ...
      for (const lt of loadTrucks) {
        ...
        if (!qty) continue;
        inQty += qty;
        if (String(lt.date || "") >= lastDate) { lastDate = String(lt.date || ""); lastFactory = Number(lt.factory_id); }
      }
...
app.put("/api/trucks/:id", requireAuth, async (req, res) => {
  try {
    ...
    await store.update("load_trucks", id, {
      ...
      truck_no: req.body.truck_no || "", date: req.body.date || t.date,
      lines: JSON.stringify(lines.map((l) => ({ model_id: Number(l.model_id), qty: Number(l.qty) }))),
      notes: req.body.notes || "",
    });
    res.json({ ok: true });
...
app.post("/api/seed", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    for (const name of SEED_LOGISTICS) {
      if (!logistics.some((l) => String(l.name).toLowerCase() === name.toLowerCase()))
    ...
    for (const name of SEED_FACTORIES) {
      if (!factories.some((f) => String(f.name).toLowerCase() === name.toLowerCase()))
    ...
    for (const [model, cbm] of SEED_MODELS) {
      if (!models.some((m) => String(m.model).toLowerCase() === model.toLowerCase()))
