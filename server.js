...
async function computeStock(u) {
  ...
  for (const model of models) {
    for (const log of logistics) {
      ...
      let lastFactory = null, lastDate = "", lastInvoice = "";
      for (const lt of loadTrucks) {
        ...
        if (!qty) continue;
        inQty += qty;
        if (String(lt.date || "") >= lastDate) { lastDate = String(lt.date || ""); lastFactory = Number(lt.factory_id); lastInvoice = lt.invoice_no || ""; }
      }
