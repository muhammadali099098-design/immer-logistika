...
function render() {
  ...
  <div class="shell">
    <aside class="sidebar">
      <div class="brand"><a class="logo" href="#" data-action="nav" data-view="summary"><img src="/logo.png" class="brand-img" alt="IMMER" /></a></div>
...
function renderLogin() {
  ...
  <div class="login-wrap">
    <div class="login-card">
      <div class="logo"><img src="/logo.png" class="brand-img" alt="IMMER" /></div>
...
function viewSummary() {
  ...
  if (tab === "loadings") {
    ...
    return `
      ...
      <div class="tbl-wrap"><table class="table">
        <thead>
          <tr><th>Фура</th><th>Дата</th>${isAdmin ? "<th>Логист</th>" : ""}<th>Завод</th><th>Инвойс</th><th>Товар</th><th>CBM</th><th></th></tr>
          <tr style="background:var(--surface-2)"><th>${mfT("truck_no", "Фура", tNoOpts)}</th><th></th>${isAdmin ? `<th>${mfT("logistics", "Логист", tLogOpts)}</th>` : ""}<th>${mfT("factory", "Завод", tFacOpts)}</th><th>${mfT("invoice", "Инвойс", tInvOpts)}</th><th></th><th></th><th></th></tr>
        </thead>
        <tbody>
          ${trucks.length === 0 ? `<tr><td colspan="${isAdmin ? 8 : 7}"><div class="emptystate">Загрузок пока нет. Нажмите «Записать загрузку».</div></td></tr>` : trucks.map((t) => {
            ...
            return `<tr>
              ...
              <td style="white-space:nowrap">
                ...
                <button class="btn link danger sm" data-action="del-truck" data-id="${t.id}">✕</button>
              </td>
            </tr>`;
          }).join("")}
        </tbody>
  ...
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      <thead>
        <tr><th>Модель</th><th>Категория</th>${isAdmin ? "<th>Логист</th>" : ""}<th>Завод</th><th>Инвойс</th><th>Пришло</th><th>Дата забора</th><th>Отгружено</th><th>Дата отправки</th><th>Остаток</th><th>Общий куб</th><th>Дней</th><th>Статус</th></tr>
        ...
        ${rows.length === 0 ? `<tr><td colspan="11"><div class="emptystate">Отгрузок пока нет.</div></td></tr>` : rows.map((s) => `
          <tr>
            ...
            <td style="white-space:nowrap">
              ...
              <button class="btn link danger sm" data-action="del-shipment" data-id="${s.id}">✕</button>
            </td>
          </tr>`).join("")}
      </tbody>
    </table></div>`;
...
function viewFinance(tabBar) {
  ...
  const totalDebtAll = groups.reduce((a, g) => a + g.total_debt, 0);
  return `${tabBar}
    <div class="card" style="margin-bottom:16px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
      ...
      <div style="font-size:18px">Общий долг: <b style="color:var(--brand)">${fmtNum(totalDebtAll)}</b></div>
    </div>
    ${groups.length === 0 ? `<div class="emptystate">Нет данных. Заполните приём в «Отгрузках», чтобы появился срок оплаты (прибытие + 30 дней).</div>` : groups.map((g) => `
      <div class="card" style="margin-bottom:16px">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:10px">
          <h2 style="margin:0">${esc(g.logistics_name)}</h2>
          <div style="font-size:16px">Долг: <b style="color:var(--brand)">${fmtNum(g.total_debt)}</b></div>
        </div>
        <div class="tbl-wrap"><table class="table" style="font-size:13px">
          <thead><tr><th>Фура</th><th>Прибытие</th><th>Оплатить до</th><th>Срок</th><th>Сумма</th><th>Оплачено</th><th>Долг</th><th>История оплат</th><th></th></tr></thead>
          <tbody>
            ${g.rows.map((r) => `
              <tr>
                <td><b>${esc(r.truck_no || "—")}</b></td>
                <td>${r.arrival ? fmtDate(r.arrival) : "—"}</td>
                <td>${r.due ? fmtDate(r.due) : "—"}</td>
                <td><b style="color:${r.days_left != null && r.days_left < 0 ? "var(--danger)" : r.days_left != null && r.days_left <= 7 ? "#b45309" : "var(--fg)"}">${r.days_left != null ? (r.days_left < 0 ? `просрочено ${-r.days_left} дн.` : `${r.days_left} дн.`) : "—"}</b></td>
                <td>${fmtNum(r.total)}</td>
                <td style="color:var(--muted)">${fmtNum(r.paid)}</td>
                <td><b>${fmtNum(r.debt)}</b></td>
                <td style="color:var(--muted)">${r.payments.map((p) => `${fmtNum(p.amount)} (${fmtDate(p.date)})`).join(", ") || "—"}</td>
                <td><button class="btn link sm" data-action="pay-ship" data-id="${r.id}">Оплатить</button></td>
              </tr>`).join("")}
          </tbody>
        </table></div>
      </div>`).join("")}`;
...
function openPayShip(id) {
  const overlay = openModal(`
    <h3>Оплата</h3>
    <form id="payShipForm" class="form" style="margin-top:14px">
      <div class="grid2">
        <div><label>Сумма *</label><input id="ps_amount" type="number" min="0" step="any" required /></div>
        <div><label>Валюта</label><select id="ps_currency">${CURRENCIES.map((c) => `<option>${c}</option>`).join("")}</select></div>
      </div>
      <div><label>Дата оплаты</label><input id="ps_date" type="date" value="${new Date().toISOString().slice(0, 10)}" /></div>
      <div><label>Документ об оплате *</label><input type="file" id="psFile" required /></div>
      <div id="psError" class="formerror"></div>
  ...
  const form = overlay.querySelector("#payShipForm");
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    const file = form.querySelector("#psFile").files[0];
    if (!file) { form.querySelector("#psError").textContent = "Загрузите документ"; return; }
    const up = await uploadFile(file);
    if (!up.ok) { form.querySelector("#psError").textContent = up.error || "Ошибка файла"; return; }
    const r = await api("/api/shipments/" + id + "/pay", { method: "POST", body: { amount: Number(form.querySelector("#ps_amount").value), currency: form.querySelector("#ps_currency").value, date: form.querySelector("#ps_date").value, file_name: up.file_name } });
    if (r.data.ok) { overlay.remove(); toast("Оплата добавлена"); await loadFinance(); render(); }
    else form.querySelector("#psError").textContent = r.data.error || "Ошибка";
  });
  [...overlay.querySelectorAll("[data-close]")].forEach((b) => b.addEventListener("click", () => overlay.remove()));
}
...
async function onClick(e) {
  ...
  switch (action) {
    ...
    case "pay-ship": openPayShip(id); break;
