...
function viewSummary() {
  ...
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      <thead><tr><th>Модель</th><th>Категория</th>${showLog ? "<th>Логист</th>" : ""}<th>CBM/шт</th><th>Остаток</th><th>Объём, м³</th><th>Пришло</th><th>Отгружено</th><th>Завод</th><th>Инвойс</th></tr></thead>
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="${showLog ? 10 : 9}"><div class="emptystate">Нет данных под фильтр. Добавьте загрузку.</div></td></tr>` : rows.map((r) => `<tr>
...
function openShipment(item) {
  ...
  let avail = availFor(editing ? Number(item.logistics_id) : (isAdmin && shLogSel ? Number(shLogSel.value) : Number(user.company_id)));
  ...
  lineArr.length = 0;
  if (editing && item.lines && item.lines.length) item.lines.forEach((li) => addLine(li, avail)); else addLine(null, avail);
