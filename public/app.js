...
function viewShipmentsTable(tabBar) {
  ...
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      <thead>
        <tr><th>Номер документа</th><th>Фура</th><th>Дата отправки</th><th>Дата прибытия</th>${isAdmin ? "<th>Логист</th>" : ""}<th>Номер PI</th><th>Товар</th><th>Стоимость</th><th>0.4%</th><th>Итого</th><th>Приём</th><th></th></tr>
        <tr style="background:var(--surface-2)">
...
function openShipment(item) {
  ...
  const overlay = openModal(`
    ...
    <form id="shipmentForm" class="form" style="margin-top:14px">
      ...
      <div class="grid2">
        ...
        <div><label>Номер документа</label><input id="sh_doc" value="${editing ? esc(item.doc_number || "") : ""}" placeholder="Добавите, когда фура выйдет на дорогу" /></div>
      </div>
      <div class="grid2">
      ...
      <div><label>Товар — можно брать из разных Order № (одна фура из разных заказов)</label><div id="sh_lines"></div>
        <button type="button" class="btn ghost sm" data-line-add>+ Добавить модель</button></div>
  ...
  const key = (o, m) => `${String(o)}__${Number(m)}`;
  ...
  // All available (order x model) combinations of a logistics with remaining qty.
  function combos(logId) {
    if (!logId) return [];
    const loaded = {}, shipped = {};
    state.trucks.filter((t) => Number(t.logistics_id) === Number(logId)).forEach((t) => {
      (t.lines || []).forEach((li) => { const k = key(t.order_no, li.model_id); loaded[k] = (loaded[k] || 0) + (Number(li.qty) || 0); });
    });
    state.shipments.filter((s) => Number(s.logistics_id) === Number(logId)).forEach((s) => {
      (s.lines || []).forEach((li) => { const k = key(li.order_no, li.model_id); shipped[k] = (shipped[k] || 0) + (Number(li.qty) || 0); });
    });
    const out = [];
    Object.keys(loaded).forEach((k) => {
      const rem = (loaded[k] || 0) - (shipped[k] || 0);
      const [o, mid] = k.split("__");
      const m = state.models.find((x) => Number(x.id) === Number(mid));
      if (rem > 0 && m) out.push({ ukey: k, order_no: o, model_id: Number(mid), model_name: m.model, qty: rem, cbm_per_pc: m.cbm_per_pc });
    });
    out.sort((a, b) => a.order_no.localeCompare(b.order_no, "nu") || a.model_name.localeCompare(b.model_name, "ru"));
    return out;
  }
  function addLine(pre) {
    const list = combos(curLog);
    if (pre) {
      const k = key(pre.order_no, pre.model_id);
      if (!list.some((x) => x.ukey === k)) {
        const m = state.models.find((x) => Number(x.id) === Number(pre.model_id));
        if (m) list.push({ ukey: k, order_no: pre.order_no || "", model_id: Number(pre.model_id), model_name: m.model, qty: 0, cbm_per_pc: m.cbm_per_pc });
      }
    }
    const row = document.createElement("div");
    row.style.cssText = "display:flex;gap:8px;margin-bottom:8px;align-items:center";
    const val = pre ? key(pre.order_no, pre.model_id) : "";
    row.innerHTML = `<select class="line-model" style="flex:1.7"><option value="">Order · Модель</option>${list.map((x) => `<option value="${esc(x.ukey)}" ${val && val === x.ukey ? "selected" : ""}>${esc(x.order_no)} · ${esc(x.model_name)} (в нал. ${fmtNum(x.qty)})</option>`).join("")}</select>
      <input class="line-qty" type="number" min="0" step="any" value="${pre ? pre.qty : ""}" placeholder="Кол-во" style="flex:1" />
      <span class="line-cbm" style="min-width:80px;color:var(--muted);font-size:12px">—</span>
      <button type="button" class="btn danger sm" data-line-del>×</button>`;
    const mSel = row.querySelector(".line-model");
    ...
    const cbmSpan = row.querySelector(".line-cbm");
    const upd = () => {
      const c = list.find((x) => x.ukey === mSel.value);
      const cb = c ? Number(c.cbm_per_pc) || 0 : 0;
      const q = Number(qIn.value) || 0;
      cbmSpan.textContent = q && cb ? (q * cb).toFixed(3) + " м³" : "—";
      let t = 0;
      lineArr.forEach((r) => {
        const rr = list.find((x) => x.ukey === r.querySelector(".line-model").value);
        t += (Number(r.querySelector(".line-qty").value) || 0) * (rr ? Number(rr.cbm_per_pc) || 0 : 0);
      });
      const tEl = document.getElementById("sh_totalCbm");
      if (tEl) tEl.textContent = "Общий CBM: " + t.toFixed(3) + " м³";
    };
    mSel.addEventListener("change", upd);
    qIn.addEventListener("input", upd);
    row.querySelector("[data-line-del]").addEventListener("click", () => { row.remove(); const i = lineArr.indexOf(row); if (i >= 0) lineArr.splice(i, 1); upd(); });
  ...
  if (shLogSel) shLogSel.addEventListener("change", () => {
    ...
    linesBox.innerHTML = ""; lineArr.length = 0;
    if (combos(curLog).length) addLine(null);
    else linesBox.innerHTML = `<div class="emptystate" style="padding:10px">У этого логиста нет доступных товаров. Сначала добавьте загрузку по Order №.</div>`;
  });
  if (editing && item.lines && item.lines.length) {
    item.lines.forEach((li) => addLine(li));
  } else if (combos(curLog).length) {
    addLine(null);
  } else {
    linesBox.innerHTML = `<div class="emptystate" style="padding:10px">Нет доступных товаров. Сначала запишите загрузку по Order №.</div>`;
  }
  form.querySelector("[data-line-add]").addEventListener("click", () => { if (combos(curLog).length) addLine(null); });
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    const lines = lineArr.map((r) => {
      const v = r.querySelector(".line-model").value;
      if (!v) return null;
      const [o, mid] = v.split("__");
      return { model_id: Number(mid), qty: Number(r.querySelector(".line-qty").value), order_no: o };
    }).filter((l) => l && l.model_id && l.qty > 0);
    const body = {
