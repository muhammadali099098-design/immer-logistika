/* IMMER Logistika — склад по моделям, PI-загрузки с фурами, отгрузки. */
"use strict";
...
async function api(path, opts = {}) {
  ...
  try {
    ...
    return { status: res.status, data: { ok: false, error: "Нет ответа от сервера, попробуйте ещё раз" } };
  }
}
...
async function loadLoadings() { const r = await api("/api/loadings"); if (r.data.ok) state.loadings = r.data.loadings; }
...
async function loadSummary() { const r = await api("/api/summary"); if (r.data.ok) state.summary = r.data.groups || []; }
async function loadAll() { await Promise.all([loadStock(), loadRefs(), loadShipments(), loadSummary()]); }
function landingView() { return state.user && state.user.role === "admin" ? "summary" : "stock"; }
...
function render() {
  ...
  const nav = [
    { v: "summary", l: "Сводка", admin: true },
    { v: "stock", l: "Склад" },
    { v: "loadings", l: "Загрузки (PI)" },
...
function renderView() {
  try {
    switch (state.view) {
      ...
      case "loadings": return viewLoadings();
...
function viewSummary() {
  const groups = state.summary;
  const grandQty = groups.reduce((a, g) => a + g.totalQty, 0);
  const grandCbm = groups.reduce((a, g) => a + g.totalCbm, 0);
  const grandPaid = groups.reduce((a, g) => a + g.totalPaid, 0);
  return `
    <div class="toolbar"><div><h1>Сводка по логистам</h1><p class="sub" style="margin:0">Какой товар у какого логиста, оплата и остаток.</p></div></div>
    <div class="grid-stats">
      <div class="card stat"><div class="num">${groups.length}</div><div class="lbl">Логистов</div></div>
      <div class="card stat"><div class="num">${fmtNum(grandQty)}</div><div class="lbl">Всего единиц на складах</div></div>
      <div class="card stat"><div class="num">${fmtNum(grandCbm)} м³</div><div class="lbl">Общий объём</div></div>
      <div class="card stat"><div class="num red">${fmtNum(grandPaid)}</div><div class="lbl">Всего оплачено логистики</div></div>
    </div>
    ${groups.length === 0 ? `<div class="emptystate">Пока нет данных. Добавьте логистов, загрузки и отгрузки.</div>`
      : groups.map((g) => `
        <div class="card" style="margin-bottom:16px">
          <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:10px;align-items:center">
            <div><h2 style="margin:0;text-transform:none">${esc(g.logistics_name)}</h2>
              <div style="color:var(--muted);font-size:13px">Загрузок: ${g.loadings} · Отгрузок: ${g.shipments} · Моделей на складе: ${g.models}</div></div>
            <div style="display:flex;gap:16px;font-size:14px">
              <div><div style="color:var(--muted);font-size:12px">Остаток</div><b>${fmtNum(g.totalQty)}</b></div>
              <div><div style="color:var(--muted);font-size:12px">Объём</div><b>${fmtNum(g.totalCbm)} м³</b></div>
              <div><div style="color:var(--muted);font-size:12px">Пришло</div>${fmtNum(g.totalReceived)}</div>
              <div><div style="color:var(--muted);font-size:12px">Отгружено</div>${fmtNum(g.totalShipped)}</div>
              <div><div style="color:var(--muted);font-size:12px">Оплачено</div><b style="color:var(--accent-fg)">${fmtNum(g.totalPaid)}</b></div>
            </div>
          </div>
          ${g.stock.length === 0 ? `<div style="margin-top:10px;color:var(--muted);font-size:13px">На складе пусто.</div>` : `
          <div style="overflow:auto;margin-top:12px"><table class="table" style="font-size:13px">
            <thead><tr><th>Модель</th><th>Остаток</th><th>Объём, м³</th><th>Пришло</th><th>Отгружено</th><th>Завод</th><th>Стоимость</th></tr></thead>
            <tbody>${g.stock.map((r) => `<tr class="${r.qty < 0 ? '' : ''}">
...
function viewStock() {
  ...
  return `
    ...
    <div class="grid-stats">
      ...
      <div class="card stat"><div class="num">${fmtNum(rows.reduce((a, r) => a + r.qty, 0))}</div><div class="lbl">Всего единиц</div></div>
      <div class="card stat"><div class="num">${fmtNum(rows.reduce((a, r) => a + r.cbm, 0))} м³</div><div class="lbl">Общий объём (CBM)</div></div>
      <div class="card stat"><div class="num red">${fmtNum(s.totalCost || 0)} ${esc(s.totalCostCurrency || "USD")}</div><div class="lbl">Потрачено на логистику</div></div>
    </div>
    <div class="tbl-wrap"><table class="table">
      <thead><tr><th>Модель</th><th>Категория</th>${isAdmin ? "<th>Логист</th>" : ""}<th>Остаток</th><th>Объём, м³</th><th>Пришло</th><th>Отгружено</th><th>Загружен с завода</th><th>Стоимость логистики</th></tr></thead>
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="${isAdmin ? 9 : 8}"><div class="emptystate">Пока нет данных. Добавьте модели, заводы и загрузки.</div></td></tr>` : rows.map((r) => {
          const risk = r.qty < 0;
          return `<tr>
            <td><b>${esc(r.model_name)}</b></td>
    ...
    <p class="sub" style="margin-top:10px">Остаток = загружено с заводов − отгружено в Узбекистан. Объём = количество × CBM/шт модели.</p>`;
}

// ---------- loadings (PI with trucks) ----------
function viewLoadings() {
  const isAdmin = state.user.role === "admin";
  return `
    <div class="toolbar"><div><h1>Загрузки с заводов</h1><p class="sub" style="margin:0">Каждая загрузка (PI) — партия с завода, разбитая на фуры.</p></div>
    <button data-action="open-loading">Новая загрузка (PI)</button></div>
    ${state.loadings.length === 0 ? `<div class="emptystate">Загрузок пока нет. Создайте первую.</div>` : state.loadings.map((l) => `
      <div class="card" style="margin-bottom:14px">
        <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px;align-items:center">
          <div>
            <b>${esc(l.pi_number || "PI")}</b> · ${esc(l.factory_name || "—")}
            ${isAdmin ? ` <span style="color:var(--muted);font-size:13px">· ${esc(l.logistics_name)}</span>` : ""}
          </div>
          <div style="display:flex;gap:8px;align-items:center">
            <span style="color:var(--muted);font-size:13px">${fmtDate(l.date)} · фур: ${l.trucks.length}</span>
            <button class="btn ghost sm" data-action="add-truck" data-id="${l.id}">+ Фура</button>
          </div>
        </div>
        ${l.notes ? `<div style="margin-top:8px;color:var(--muted);font-size:13px">${esc(l.notes)}</div>` : ""}
        ${l.trucks.length === 0 ? `<div style="margin-top:10px;color:var(--muted);font-size:13px">Фур ещё нет — добавьте первую.</div>` : `
        <div style="overflow:auto;margin-top:10px"><table class="table" style="font-size:13px">
          <thead><tr><th>Фура</th><th>Дата</th><th>Товар</th></tr></thead>
          <tbody>${l.trucks.map((t) => `<tr>
            <td><b>${esc(t.truck_no || "—")}</b></td>
            <td style="color:var(--muted)">${fmtDate(t.date)}</td>
            <td>${t.lines.map((li) => `${esc(li.model_name)} × <b>${fmtNum(li.qty)}</b>`).join(" · ") || "—"}</td>
          </tr>`).join("")}</tbody>
        </table></div>`}
      </div>`).join("")}`;
}
...
function openLoading() {
  ...
  const logCmd = isAdmin
    ? `<select id="lg_logistic"><option value="">Выберите логиста</option>${state.logistics.map((l) => `<option value="${l.id}">${esc(l.name)}</option>`).join("")}</select>`
    : `<input value="${esc(state.user.company_name || "")}" disabled />`;
  const overlay = openModal(`
    <h3>Новая загрузка (PI)</h3>
    <form id="loadingForm" class="form" style="margin-top:14px">
      ${isAdmin ? `<div><label>Логист</label>${logCmd}</div>` : `<div><label>Логист</label>${logCmd}<input type="hidden" id="lg_logistic" value="${logId}" /></div>`}
      <div class="grid2">
        <div><label>Завод</label><select id="lg_factory"><option value="">Выберите завод</option>${state.factories.map((f) => `<option value="${f.id}">${esc(f.name)}</option>`).join("")}</select></div>
        <div><label>Номер PI</label><input id="lg_pi" placeholder="Например: HD250429" /></div>
      </div>
      <div class="grid2">
        <div><label>Дата</label><input id="lg_date" type="date" value="${new Date().toISOString().slice(0, 10)}" /></div>
        <div><label>Примечание</label><input id="lg_notes" /></div>
      </div>
      <div id="loadingError" class="formerror"></div>
      <div class="actions"><button type="button" class="btn ghost" data-close>Отмена</button><button type="submit">Создать загрузку</button></div>
    </form>`);
  const form = overlay.querySelector("#loadingForm");
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    const body = {
      logistics_id: Number(form.querySelector("#lg_logistic").value),
      factory_id: Number(form.querySelector("#lg_factory").value),
      pi_number: form.querySelector("#lg_pi").value,
      date: form.querySelector("#lg_date").value,
      notes: form.querySelector("#lg_notes").value,
    };
    const r = await api("/api/loadings", { method: "POST", body });
    if (r.data.ok) { overlay.remove(); toast("Загрузка создана"); await loadLoadings(); render(); }
    else form.querySelector("#loadingError").textContent = r.data.error || "Ошибка";
  });
  [...overlay.querySelectorAll("[data-close]")].forEach((b) => b.addEventListener("click", () => overlay.remove()));
}

function addTruck(loadingId) {
  const overlay = openModal(`
    <h3>Добавить фуру</h3>
    <form id="truckForm" class="form" style="margin-top:14px">
      <div class="grid2">
        <div><label>Номер фуры</label><input id="tr_no" placeholder="Например: 202AOH08" /></div>
        <div><label>Дата загрузки</label><input id="tr_date" type="date" value="${new Date().toISOString().slice(0, 10)}" /></div>
      </div>
      ...
      <div class="actions"><button type="button" class="btn ghost" data-close>Отмена</button><button type="submit">Сохранить фуру</button></div>
    </form>`);
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const body = { truck_no: form.querySelector("#tr_no").value, date: form.querySelector("#tr_date").value, lines };
    const r = await api("/api/loadings/" + loadingId + "/trucks", { method: "POST", body });
    if (r.data.ok) { overlay.remove(); toast("Фура добавлена"); await loadLoadings(); render(); }
...
function viewShipments() {
  const isAdmin = state.user.role === "admin";
  return `
    <div class="toolbar"><div><h1>Отгрузки в Узбекистан</h1><p class="sub" style="margin:0">Фуры со склада. За каждую — стоимость логистики.</p></div>
    ...
    ${state.shipments.length === 0 ? `<div class="emptystate">Отгрузок пока нет.</div>` : state.shipments.map((s) => `
      <div class="card" style="margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px;align-items:center">
          ...
          <div><b style="color:var(--accent-fg)">${fmtNum(s.cost_amount)} ${esc(s.cost_currency)}</b></div>
        </div>
  ...
  const overlay = openModal(`
    ...
    <form id="modelForm" class="form" style="margin-top:14px">
      ...
      <div class="grid2">
        ...
        <div><label>CBM за 1 шт *</label><input id="mo_cbm" type="number" step="0.0001" value="${esc(m ? m.cbm_per_pc : "")}" placeholder="0.112685625" /></div>
      </div>
...
async function go(view) {
  ...
  try {
    ...
    else if (view === "stock") { await Promise.all([loadStock(), loadStats()]); }
    else if (view === "loadings") await loadLoadings();
...
async function onClick(e) {
  ...
  switch (action) {
    ...
    case "open-loading": openLoading(); break;
    case "add-truck": addTruck(id); break;
