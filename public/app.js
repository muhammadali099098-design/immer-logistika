      ...
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="14"><div class="emptystate">Отгрузок пока нет.</div></td></tr>` : rows.map((s) => {
          ...
          <tr>
            ...
            <td style="max-width:220px">${linesLabel(s.lines)}</td>
          ...
          <tbody>
            ${g.rows.map((r) => `
              <tr>
                ...
                <td style="max-width:220px">${linesLabel(r.lines)}</td>
...
function openShipment(item) {
  ...
  function pickGoods() {
    ...
    orders.forEach((o) => {
      ...
      html += `<table class="table" style="font-size:13px"><thead><tr><th></th><th>Модель</th><th>Остаток</th><th>Кол-во</th></tr></thead><tbody>`;
      html += items.map((k) => {
