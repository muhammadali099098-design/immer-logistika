...
async function api(path, opts = {}) {
  ...
  try {
    ...
    // Network failure (server waking up, timeout, offline) — never fail silently.
...
async function loadStock() {
  ...
  if (r.data.ok) state.stock = r.data.rows || [];
}
...
async function boot() {
  ...
  if (r.data.ok && r.data.user) {
    ...
    state.view = "stock";
    render(); // open the app immediately, then load data in the background
    await loadAll();
    render();
  }
}
...
async function go(view) {
  // Switch the view instantly (no waiting for the server), then refresh data
  // in the background. Free-tier Render can be slow to wake, so the page
  // must never appear "stuck" on a click.
  state.view = view;
  render();
  state.loading = true;
  try {
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", async (e) => {
      ...
      if (r.data.ok) {
        ...
        render(); // enter the app immediately, load data behind it
