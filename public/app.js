...
const state = {
  ...
  finance: [],
  navCollapsed: false,
  navMobile: false, // мобильная "шторка" меню
  finLogId: "",
  shipFilters: {},
  finCol: {},
  bulkSel: new Set(),
...
function isMobileView() { return (window.innerWidth || 0) <= 860; }
...
function render() {
  ...
  app.innerHTML = `
  <div class="shell ${state.navCollapsed ? "nav-collapsed" : ""} ${state.navMobile ? "nav-mobile-open" : ""}">
    ${state.navMobile ? `<div class="nav-backdrop" data-action="toggle-nav"></div>` : ""}
    <aside class="sidebar">
    ...
    <div class="main">
      <div class="topbar">
        <div style="display:flex;align-items:center;gap:12px">
          <button class="btn ghost sm" data-action="toggle-nav" title="${isMobile ? "Открыть меню" : "Свернуть / развернуть панель"}">${isMobile || state.navCollapsed ? "☰" : "«"}</button>
...
async function onClick(e) {
  ...
  switch (action) {
    ...
    case "nav": e.preventDefault(); state.navMobile = false; go(el.getAttribute("data-view")); break;
    ...
    case "toggle-nav": if (isMobileView()) { state.navMobile = !state.navMobile; render(); } else { state.navCollapsed = !state.navCollapsed; render(); } break;
