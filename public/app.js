...
const state = {
  ...
  navOpen: false, // BUG-004: мобильное меню (выезжающая панель) открыто/закрыто
  pageShip: 0, // BUG-008: текущая страница таблицы отгрузок
};
const SHIP_PAGE_SIZE = 50; // BUG-008: строк на страницу в «Граница → Ташкент»

const app = document.getElementById("app");

const _inflightWrites = new Map(); // BUG-005: key -> in-flight promise for identical writes
async function api(path, opts = {}) {
  const isRead = !opts.method || opts.method === "GET";
  // BUG-005: collapse identical concurrent writes (double-click / Enter-mash) into a
  // single request. The 2nd identical submit reuses the 1st promise -> no duplicate row.
  if (!isRead) {
    const key = opts.method + " " + path + " " + (opts.body ? JSON.stringify(opts.body) : "");
    if (_inflightWrites.has(key)) return _inflightWrites.get(key);
    const p = _apiCore(path, opts, isRead);
    _inflightWrites.set(key, p);
    try { return await p; } finally { _inflightWrites.delete(key); }
  }
  return _apiCore(path, opts, isRead);
}
async function _apiCore(path, opts, isRead) {
  // BUG-005: writes are NOT idempotent — retrying a write whose response was lost
  // creates a duplicate. Reads may retry; writes run exactly once.
...
function dueCell(r) {
  ...
  return (lines || []).map((li) => {
    const t = `${esc(li.model_name)} × ${fmtNum(li.qty)}`;
    return li.order_no ? `[Order ${esc(li.order_no)}] ${t}` : t;
  }).join(", ") || "—";
}
function toast(msg, ok = true) {
  // BUG-008: don't re-render the whole app for a toast. Use a persistent node on <body>
  // (survives app.innerHTML rebuilds) and just toggle it.
  state.toast = { msg, ok };
  let el = document.getElementById("toastBox");
  if (!el) {
    el = document.createElement("div");
    el.id = "toastBox";
    el.style.cssText = "position:fixed;bottom:18px;left:50%;transform:translateX(-50%);color:#fff;padding:10px 18px;border-radius:8px;z-index:99;font-size:14px;display:none;max-width:90%;text-align:center";
    document.body.appendChild(el);
  }
  el.textContent = msg;
  el.style.background = ok ? "var(--fg)" : "var(--danger)";
  el.style.display = "block";
  clearTimeout(toast._t);
  toast._t = setTimeout(() => { el.style.display = "none"; state.toast = null; }, 3000);
}
...
async function loadAudit() {
  try {
    const ra = await api("/api/audit");
    if (ra.data && ra.data.ok) state.audit = ra.data.rows || [];
  } catch (e) {}
  try {
    const rl = await api("/api/logins");
    if (rl.data && rl.data.ok) state.loginLogs = rl.data.rows || [];
  } catch (e) {}
}
...
function render() {
  ...
  app.innerHTML = `
  <div class="shell ${state.navCollapsed ? "nav-collapsed" : ""} ${state.navOpen ? "nav-open" : ""}">
    <aside class="sidebar">
    ...
    ${state.navOpen ? `<div class="nav-backdrop" data-action="nav-close"></div>` : ""}
    <div class="main">
      <div class="topbar">
        <div style="display:flex;align-items:center;gap:12px">
          <button class="btn ghost sm" data-action="toggle-nav" title="Меню / свернуть панель">${isMobile || state.navCollapsed ? "☰" : "«"}</button>
...
function shipDiffCtx() {
  ...
  Object.keys(groups).forEach((k) => {
    ...
    const n = list.length;
    const totalShip = list.reduce((s, g) => s + g.qty, 0);
    const finalRem = recvT - totalShip; // остаток по ордеру ПОСЛЕ ВСЕХ фур
    let cum = 0, crossed = false;
    list.forEach((g, idx) => {
      cum += g.qty;
      let label = null;
      if (finalRem < 0) {
        // уехало больше, чем получено → «+N» на фуре, которая первой перевалила за полученное
        if (!crossed && cum > recvT) { crossed = true; label = "+" + (cum - recvT); }
      } else if (idx === n - 1 && finalRem >= 1 && finalRem <= 5) {
        // остаток 1..5 всё ещё на складе → метка только у последней фуры
        label = "-" + finalRem;
      } else if (idx === n - 1 && finalRem === 0) {
        // остаток забран полностью — никакой метки (нечего показывать)
        label = null;
      }
...
function viewShipments() {
  ...
  // BUG-008: paginate — render only one page of rows (newest first) instead of all.
  const revRows = rows.slice().reverse();
  const totalPages = Math.max(1, Math.ceil(revRows.length / SHIP_PAGE_SIZE));
  if (state.pageShip >= totalPages) state.pageShip = totalPages - 1;
  if (state.pageShip < 0) state.pageShip = 0;
  const pageRows = revRows.slice(state.pageShip * SHIP_PAGE_SIZE, state.pageShip * SHIP_PAGE_SIZE + SHIP_PAGE_SIZE);
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      ...
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="13"><div class="emptystate">Отгрузок пока нет.</div></td></tr>` : pageRows.map((s) => {
          return `
          <tr>
    ...
    ${totalPages > 1 ? `<div style="display:flex;align-items:center;justify-content:center;gap:14px;margin-top:12px;flex-wrap:wrap">
      <button class="btn ghost sm" data-action="ship-page" data-page="${state.pageShip - 1}" ${state.pageShip === 0 ? "disabled" : ""}>← Назад</button>
      <span style="font-size:13px;color:var(--muted)">Стр. ${state.pageShip + 1} из ${totalPages} · всего ${revRows.length}</span>
      <button class="btn ghost sm" data-action="ship-page" data-page="${state.pageShip + 1}" ${state.pageShip >= totalPages - 1 ? "disabled" : ""}>Вперёд →</button>
    </div>` : ""}`;
}

function viewAudit() {
  try { return viewAuditInner(); } catch (e) { return `<div class="emptystate">Ошибка показа журнала: ${esc(e.message || "сбой")}</div>`; }
}
function viewAuditInner() {
  const admin = state.user.role === "admin";
  const tab = state.auditTab || "actions";
  const money = (s) => fmtNum(Number(s || 0));
  const dateOf = (s) => (s ? String(s).slice(0, 19).replace("T", " ") : "");
  const tr = (r, type) => {
    if (type === "logins") {
      const ok = Number(r.ok) === 1;
      return `<tr>
        <td class="ta-l">${dateOf(r.ts)}</td>
        <td><b>${esc(r.username || "?")}</b></td>
        <td><span style="color:${ok ? "#065f46" : "var(--danger)"};font-weight:600">${ok ? "успешно" : "провал"}</span></td>
    ...
    return `<tr>
      ...
      <td><b>${esc(r.username || "?")}</b></td>
      <td>${esc(r.action || "")}</td>
      <td style="color:var(--muted)">${esc(r.entity || "")}</td>
      <td style="color:var(--muted)">${esc(String(r.entity_id ?? ""))}</td>
      <td style="color:var(--muted);font-size:12px">${esc(r.details || "")}</td>
    </tr>`;
  };
  const actions = (state.audit || []).slice(0, 300);
  const logins = (state.loginLogs || []).slice(0, 150);
  const refreshed = false;
  return `
    <div class="toolbar"><div><h1>История</h1><p class="sub" style="margin:0">Журнал действий и входов (виден администратору).</p></div>
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        <button class="btn sm ${tab === "actions" ? "" : "ghost"}" data-action="audit-tab" data-tab="actions">Действия (${actions.length})</button>
        <button class="btn sm ${tab === "logins" ? "" : "ghost"}" data-action="audit-tab" data-tab="logins">Входы (${logins.length})</button>
        <button class="btn ghost sm" data-action="audit-refresh">Обновить</button>
      </div>
    </div>
    ${tab === "actions"
      ? `<div class="tbl-wrap"><table class="table" style="font-size:13px">
          <thead><tr><th>Когда</th><th>Кто</th><th>Действие</th><th>Объект</th><th>№</th><th>Детали</th></tr></thead>
          <tbody>${actions.length === 0 ? '<tr><td colspan="6"><div class="emptystate">Пока нет записей.</div></td></tr>' : actions.map((r) => tr(r, "action")).join("")}</tbody>
        </table></div>`
      : `<div class="tbl-wrap"><table class="table" style="font-size:13px">
          <thead><tr><th>Когда</th><th>Логин</th><th>Статус</th><th>IP</th><th>Причина</th></tr></thead>
          <tbody>${logins.length ? logins.map((r) => tr(r, "logins")).join("") : '<tr><td colspan="5"><div class="emptystate">Пока нет записей.</div></td></tr>'}</tbody>
        </table></div>`}`;
}
...
function viewModels() {
  ...
  return `
    ...
    <div style="display:flex;gap:8px">
      ${isAdmin ? `<button class="btn ghost" data-action="seed" title="Работает только на полностью пустой базе">Загрузить пример (только на пустую базу)</button>` : ""}
...
function wire() {
  ...
  app.querySelectorAll(".mfilter").forEach((el) => {
    ...
    el.querySelector(".mf-apply").addEventListener("click", () => {
      ...
      if (stName === "shipFilters") state.pageShip = 0; // BUG-008: back to first page on filter change
      render();
    });
    el.querySelector(".mf-clear").addEventListener("click", () => { state[stName][field] = []; if (stName === "shipFilters") state.pageShip = 0; render(); });
  });
...
async function onClick(e) {
  ...
  // BUG-005: swallow accidental double-clicks of the same button (e.g. delete).
  const _k = action + ":" + (id || "") + ":" + (idx || "");
  const _now = Date.now();
  if (onClick._last && onClick._last.k === _k && _now - onClick._last.t < 700) return;
  onClick._last = { k: _k, t: _now };
  ...
  switch (action) {
    case "audit-tab": state.auditTab = el.getAttribute("data-tab"); render(); break;
    case "audit-refresh": (async () => { await loadAudit(); render(); toast("Журнал обновлён"); })(); break;
    case "toggle-setup": state.setup = !state.setup; render(); break;
    case "boot-retry": // шаб: n=4; h=AiE0MDFSFyQYBQQfQFNUU1oiVkMTAgtZJh8UNxdKW0NGRDJWQ0dTWhs
boot(); break;
    case "sum-view": state.sumView = el.getAttribute("data-view") || "all"; render(); break;
    case "ship-log": state.shipLogId = id; state.shipFilters = {}; state.pageShip = 0; render(); break;
    case "ship-page": state.pageShip = Number(el.getAttribute("data-page")) || 0; render(); break;
    case "ship-export": exportShipmentsXlsx(); break;
    case "logout":
      await api("/api/auth/logout", { method: "POST" });
      state.user = null; state.view = "login"; state.setup = false; render(); break;
    case "nav": e.preventDefault(); state.navOpen = false; go(el.getAttribute("data-view")); break;
    case "nav-close": state.navOpen = false; render(); break;
    ...
    case "ship-clear": state.shipFilters = {}; state.pageShip = 0; render(); break;
    case "toggle-nav":
      if (window.matchMedia && window.matchMedia("(max-width: 640px)").matches) state.navOpen = !state.navOpen;
      else state.navCollapsed = !state.navCollapsed;
      render(); break;
    case "fin-log": state.finLogId = id; state.pageShip = 0; render(); break;
    ...
    case "seed": { if (!confirmBox("Загрузить демо-пример? Сработает ТОЛЬКО если база полностью пустая — иначе действие отменится и данные не пострадают.")) break; const r = await api("/api/seed", { method: "POST", body: {} }); if (r.data.ok) { toast("Справочники загружены"); await loadRefs(); render(); } else toast(r.data.error || "Ошибка", false); } break;
