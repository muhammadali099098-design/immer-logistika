      ...
      <div class="card" style="margin-bottom:16px">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:10px">
          ...
          <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
            <div style="font-size:14px">Оплачено: <b>${fmtNum(g.total_paid || 0)}</b></div>
            <div style="font-size:16px">Долг: <b style="color:var(--brand)">${fmtNum(g.total_debt)}</b></div>
            <button class="btn sm" data-action="pay-logistic" data-id="${g.logistics_id}">Внести оплату</button>
          </div>
        </div>
        <div class="tbl-wrap"><table class="table" style="font-size:13px">
          <thead><tr><th>Фура</th><th>Прибытие</th><th>Оплатить до</th><th>Срок</th><th>Сумма</th><th>Оплачено</th><th>Долг</th><th>История оплат</th></tr></thead>
          <tbody>
            ${g.rows.map((r) => `
              <tr>
...
function openPayLogistic(logisticsId) {
  const overlay = openModal(`
    <h3>Внести оплату</h3>
    <form id="payLogForm" class="form" style="margin-top:14px">
      <div class="grid2">
        <div><label>Сумма *</label><input id="pl_amount" type="number" min="0" step="any" required /></div>
        <div><label>Валюта</label><select id="pl_currency">${CURRENCIES.map((c) => `<option>${c}</option>`).join("")}</select></div>
      </div>
      <div><label>Дата оплаты</label><input id="pl_date" type="date" value="${new Date().toISOString().slice(0, 10)}" /></div>
      <div><label>Документ об оплате *</label><input type="file" id="plFile" required /></div>
      <div id="plError" class="formerror"></div>
  ...
  const form = overlay.querySelector("#payLogForm");
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    const file = form.querySelector("#plFile").files[0];
    if (!file) { form.querySelector("#plError").textContent = "Загрузите документ"; return; }
    const up = await uploadFile(file);
    if (!up.ok) { form.querySelector("#plError").textContent = up.error || "Ошибка файла"; return; }
    const r = await api("/api/payments", { method: "POST", body: { logistics_id: logisticsId, amount: Number(form.querySelector("#pl_amount").value), currency: form.querySelector("#pl_currency").value, date: form.querySelector("#pl_date").value, file_name: up.file_name } });
    if (r.data.ok) { overlay.remove(); toast("Оплата добавлена"); await loadFinance(); render(); }
    else form.querySelector("#plError").textContent = r.data.error || "Ошибка";
  });
  [...overlay.querySelectorAll("[data-close]")].forEach((b) => b.addEventListener("click", () => overlay.remove()));
}
...
async function onClick(e) {
  ...
  switch (action) {
    ...
    case "pay-logistic": openPayLogistic(id); break;
