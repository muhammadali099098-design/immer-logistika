...
function viewSummary() {
  ...
  const hasF = Object.keys(F).some((k) => F[k]);
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      <thead>
        ...
        <tr style="background:var(--surface-2)">
          ...
          <th>${sel("category", cats, "Все")}</th>
          ${isAdmin ? `<th>${sel("logistics", state.logistics.map((l) => l.name), "Все")}</th>` : ""}
          <th>${sel("factory", facs, "Все")}</th>
          <th><input class="f-input sum-filter" data-f="invoice" placeholder="Инвойс…" value="${esc(F.invoice || "")}" /></th>
          <th></th><th></th><th></th><th></th><th></th><th></th><th></th>
          <th>${sel("status", ["отправлен", "частично", "в складе"], "Все")}</th>
        </tr>
      </thead>
      <tbody>
