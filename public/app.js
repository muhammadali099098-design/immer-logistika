...
function viewSummary() {
  ...
  rows = rows.filter((r) => rngOK("received", r.received) && rngOK("shipped", r.shipped) && rngOK("qty", r.qty) && rngOK("totalCbm", (r.qty * (Number(r.cbm_per_pc) || 0)).toFixed(2)) && rngOK("days", r.days_sitting) && rngOK("pickup_date", r.pickup_date) && rngOK("send_date", r.send_date_tashkent));
  ...
  return `
    ...
    ${hasF || q ? `<button class="btn ghost" data-action="summary-clear">Сбросить фильтры</button>` : ""}</div>
    ${tabBar}
    <div class="tbl-wrap"><table class="table">
      <thead>
        ...
        <tr style="background:var(--surface-2)">
          ...
          <th>${rng("received", "number")}</th>
          <th>${rng("pickup_date", "date")}</th>
          <th>${rng("shipped", "number")}</th>
          <th>${rng("send_date", "date")}</th>
          <th>${rng("qty", "number")}</th>
          <th>${rng("totalCbm", "number")}</th>
...
function viewShipments() {
  ...
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      <thead><tr><th>Фура</th><th>Дата отправки</th><th>Дата прибытия</th>${isAdmin ? "<th>Логист</th>" : ""}<th>Номер PI</th><th>Номер документа</th><th>Товар</th><th>Стоимость</th><th>0.4%</th><th>Итого</th><th>Приём</th><th></th></tr></thead>
      <tbody>
        ${state.shipments.length === 0 ? `<tr><td colspan="11"><div class="emptystate">Отгрузок пока нет.</div></td></tr>` : state.shipments.map((s) => `
          <tr>
...
function wire() {
  ...
  app.querySelectorAll(".mfilter").forEach((el) => {
    ...
    const stName = el.getAttribute("data-store") === "truck" ? "truckFilters" : "sumFilters";
