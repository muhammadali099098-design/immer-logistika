...
function viewSummary() {
  ...
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      <thead>
        <tr><th>Order №</th><th>Модель</th><th>Категория</th><th>Логист</th><th>Завод</th><th>Инвойс</th><th>Пришло</th><th>Дата забора</th><th>Отгружено</th><th>Остаток</th><th>Общий куб</th><th>Дней</th><th>Статус</th><th>Действия</th></tr>
...
function viewShipments() {
  ...
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      <thead>
        <tr><th>Номер документа</th><th>Фура</th><th>Дата отправки</th><th>Дата прибытия</th><th>Логист</th><th>Номер PI</th><th>Товар</th><th>Куб</th><th>Стоимость</th><th>0.4%</th><th>Итого</th><th>Приём</th><th></th></tr>
        <tr style="background:var(--surface-2)">
      ...
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="12"><div class="emptystate">Отгрузок пока нет.</div></td></tr>` : rows.map((s) => {
          ...
          <tr>
            ...
            <td><button class="btn link sm" data-action="ship-goods" data-id="${s.id}">посмотреть</button></td>
    ...
    ${groups.length === 0 ? `<div class="emptystate">Нет прибывших фур. Заполните приёмку в «Граница → Ташкент».</div>` : groups.map((g) => `
      <div class="card" style="margin-bottom:16px">
        ...
        <div class="tbl-wrap"><table class="table" style="font-size:13px">
          <thead><tr><th></th><th>Номер документа</th><th>Фура</th><th>Дата прибытия</th><th>Товар</th><th>Итого</th><th>Оплачено</th><th>Долг</th><th>Дней до оплаты</th><th>История оплат</th><th></th></tr></thead>
          <tbody>
            ${g.rows.map((r) => `
              <tr>
                ...
                <td><button class="btn link sm" data-action="ship-goods" data-id="${r.shipment_id}">посмотреть</button></td>
...
async function go(view) {
  ...
  try {
    ...
    else if (view === "finance") { await Promise.all([loadShipments(), loadFinance(), loadRefs()]); }
