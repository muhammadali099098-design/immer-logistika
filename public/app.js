...
function viewSummary() {
  ...
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      <thead>
        <tr><th>Order №</th><th>Модель</th><th>Категория</th><th>Логист</th><th>Завод</th><th>Инвойс</th><th>Пришло</th><th>Дата забора</th><th>Отгружено</th><th>Остаток</th><th>Общий куб</th><th>Дней</th><th>Статус</th></tr>
      ...
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="13"><div class="emptystate">Нет данных под фильтр. Добавьте загрузку.</div></td></tr>` : rows.map((r) => {
...
function viewShipments() {
  ...
  if (SF.paid && SF.paid.length) rows = rows.filter((s) => {
    const fr = frByShip[s.id] || {};
    const debt = fr.debt != null ? fr.debt : s.total_cost;
    return SF.paid.includes(debt <= 0 ? "оплачен" : "не оплачен");
  });
  ...
  const payOpts = ["оплачен", "не оплачен"];
  ...
  const totalPaidAll = shown.reduce((a, g) => a + (g.total_paid || 0), 0);
  const totalDebtAll = shown.reduce((a, g) => a + (g.rows || []).reduce((b, r) => b + (r.debt || 0), 0), 0);
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      <thead>
        <tr><th>Номер документа</th><th>Фура</th><th>Дата отправки</th><th>Дата прибытия</th><th>Логист</th><th>Номер PI</th><th>Товар</th><th>Стоимость</th><th>0.4%</th><th>Итого</th><th>Оплачено</th><th>Долг</th><th>Приём</th><th></th></tr>
        <tr style="background:var(--surface-2)">
          ...
          <th>${mfS("paid", "Платёж", payOpts)}</th>
          <th>${mfS("recv", "Приём", recvOpts)}</th><th></th>
        </tr>
      </thead>
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="14"><div class="emptystate">Отгрузок пока нет.</div></td></tr>` : rows.map((s) => {
          const fr = frByShip[s.id] || {};
          const debt = (fr.debt != null ? fr.debt : s.total_cost);
          const paid = fr.paid || 0;
          const payStatus = debt <= 0 ? "оплачен" : "не оплачен";
          return `
          <tr>
            ...
            <td><b>${fmtNum(fr.paid || 0)}</b></td>
            <td><b style="color:var(--brand)">${fmtNum(debt)}</b></td>
            ...
            <td style="white-space:nowrap">
              ...
              <button class="btn link sm" title="Оплатить по документу" data-action="pay-doc" data-doc="${esc(s.doc_number || "")}" data-id="${s.id}" style="${s.receipt_status === "received" ? "" : "opacity:.45"}">${ICON_PAY}</button>
              <button class="btn link sm" title="Отметить приём (поступление)" data-action="receive-shipment" data-id="${s.id}">${ICON_RECV}</button>
              <button class="btn link sm" title="Документы партии" data-action="docs-shipment" data-id="${s.id}">${ICON_DOC} Документы</button>
...
function viewFinance(tabBar) {
  ...
  return `${tabBar}
    ...
    ${groups.length === 0 ? `<div class="emptystate">Нет данных. Запишите отгрузку, чтобы появились фуры с номером документа.</div>` : groups.map((g) => `
      <div class="card" style="margin-bottom:16px">
        ...
        <div class="tbl-wrap"><table class="table" style="font-size:13px">
          <thead><tr><th>Номер документа</th><th>Фура</th><th>Дата</th><th>Товар</th><th>Стоимость</th><th>Оплачено</th><th>Долг</th><th>История оплат</th><th></th></tr></thead>
          <tbody>
            ${g.rows.map((r) => `
              <tr>
                ...
                <td style="white-space:nowrap">
                  <button class="btn link sm" data-action="edit-shipment" data-id="${r.shipment_id}">✎</button>
                  <button class="btn link sm" title="Документы партии" data-action="docs-shipment" data-id="${r.shipment_id}">${ICON_DOC} Документы</button>
...
function openDocs(id) {
  ...
  const docs = [];
  if (s.receipt_file) docs.push({ label: "Приём груза (документ прибытия)", name: s.receipt_file });
  if (s.payment_status === "paid" && s.payment_file) docs.push({ label: `Отметка об оплате фуры${s.payment_date ? ` (${fmtDate(s.payment_date)})` : ""}`, name: s.payment_file });
  (fr && fr.payments ? fr.payments : []).forEach((p, i) => {
    if (p.file_name) docs.push({ label: `Платёж ${i + 1} — ${fmtNum(p.amount)} ${p.currency || ""} (${fmtDate(p.date)})`, name: p.file_name });
  });
...
function openReceive(id) {
  ...
  const recvDocHtml = already && s.receipt_file
    ? `<div style="margin-bottom:10px">${fileLink(s.receipt_file, "Открыть документ о получении")}</div><div><label>Заменить документ (необязательно)</label><input type="file" id="recvFile" /></div>`
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!file && !(already && s.receipt_file)) { form.querySelector("#recvError").textContent = "Загрузите документ — без него прибытие отметить нельзя"; return; }
    const up = file ? await uploadFile(file) : null;
    if (file && !up.ok) { form.querySelector("#recvError").textContent = up.error || "Ошибка загрузки файла"; return; }
    const body = {
      receipt_status: "received",
      file_name: up ? up.file_name : (s.receipt_file || ""),
...
function openShipment(item) {
  ...
  function pickGoods() {
    ...
    const pk = openModal(`
      ...
      <div style="margin:14px 0"><label>Order №</label><select id="pgOrder" class="f-input">${orders.map((o) => `<option>${esc(o)}</option>`).join("")}</select></div>
      <div id="pgList" style="max-height:340px;overflow:auto;border:1px solid var(--border);border-radius:8px"></div>
    ...
    `, "modal-wide");
    const orderSel = pk.querySelector("#pgOrder");
    ...
    function renderModels() {
      ...
      const chosen = new Map(); sel.forEach((x) => { if (x.order_no === o && Number(x.qty) > 0) chosen.set(x.ukey, x); });
      const items = Object.keys(st).filter((k) => k.startsWith(o + "__") && (st[k] || 0) > 0);
      if (!items.length) { listEl.innerHTML = `<div class="emptystate" style="padding:10px">В этом заказе нет доступных товаров.</div>`; return; }
      listEl.innerHTML = `<table class="table" style="font-size:13px">
        <thead><tr><th></th><th>Модель</th><th>Остаток</th><th>Кол-во</th></tr></thead><tbody>` + items.map((k) => {
        const [o2, mid] = k.split("__");
        const m = state.models.find((x) => Number(x.id) === Number(mid));
        const already = chosen.get(k);
    ...
    orderSel.addEventListener("change", renderModels);
    renderModels();
    addBtn.addEventListener("click", () => {
      const o = orderSel.value;
      const rows = listEl.querySelectorAll("tr[data-k]");
      const picks = [];
      let bad = false;
      rows.forEach((tr) => {
        ...
        const [mid] = k.split("__").slice(1);
        const m = state.models.find((x) => Number(x.id) === Number(mid));
        picks.push({ ukey: k, order_no: o, model_id: Number(mid), model_name: m ? m.model : mid, qty: q, cbm_per_pc: m ? m.cbm_per_pc : 0 });
      });
      if (bad) return;
      if (!picks.length) { errEl.textContent = "Отметьте товар и укажите количество."; return; }
      sel = sel.filter((x) => x.order_no !== o);
      picks.forEach((p) => sel.push(p));
      pk.remove();
      renderLines();
    });
