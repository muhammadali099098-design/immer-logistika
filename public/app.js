...
function viewSummary() {
  ...
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      <thead><tr><th>Модель</th><th>Категория</th>${showLog ? "<th>Логист</th>" : ""}<th>CBM/шт</th><th>Остаток</th><th>Пришло</th><th>Отгружено</th><th>Завод</th><th>Инвойс</th><th>Дата забора</th><th>Дата отправки в Ташкент</th><th>Дней на складе</th></tr></thead>
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="${showLog ? 13 : 12}"><div class="emptystate">Нет данных под фильтр. Добавьте загрузку.</div></td></tr>` : rows.map((r) => `<tr>
...
function viewShipments() {
  ...
  return `
    <div class="toolbar"><div><h1>Сводка: Граница → Ташкент</h1><p class="sub" style="margin:0">Фуры, отправленные из границы в Ташкент. Вы платите за каждую отправленную фуру.</p></div>
...
function openShipment(item) {
  ...
  if (shLogSel) shLogSel.addEventListener("change", () => {
    const na = availFor(Number(shLogSel.value));
    form.querySelectorAll("#sh_lines .line-model").forEach((sel) => {
      const cur = sel.value;
      sel.innerHTML = "";
      const o0 = document.createElement("option"); o0.value = ""; o0.textContent = "Модель"; sel.appendChild(o0);
      na.forEach((a) => { const o = document.createElement("option"); o.value = a.id; o.textContent = `${a.name} (в нал. ${fmtNum(a.qty)})`; if (String(a.id) === String(cur)) o.selected = true; sel.appendChild(o); });
      if (cur && !na.some((a) => String(a.id) === String(cur))) { const m = state.models.find((x) => Number(x.id) === Number(cur)); if (m) { const o = document.createElement("option"); o.value = cur; o.textContent = m.model; o.selected = true; sel.appendChild(o); } }
    });
  });
  function addLine(pre, availList) {
    const row = document.createElement("div");
