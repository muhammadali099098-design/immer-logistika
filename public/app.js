...
function exportFinancesCSV() {
  ...
  const rows = [["Логист", "Номер документа", "Фура", "Дата прибытия", "Инвойс (завод)", "Товар", "Итого", "Оплачено", "Долг", "Дней до оплаты"]];
  groups.forEach((g) => (g.rows || []).forEach((r) => {
    const iv = invoiceCodes(g.logistics_id, r.lines);
    const d = daysToDue(r.arrival_date);
    rows.push([g.logistics_name, r.doc_number, r.truck_no, r.arrival_date, iv.join(", "), (r.lines || []).map((l) => (l.model_name || "") + "×" + l.qty).join(", "), r.total, r.paid, r.debt, d == null ? "" : d]);
  }));
  downloadCsv("finansy_logisty", rows.map((r) => r.map((v) => '"' + String(v == null ? "" : v).replace(/"/g, '""') + '"').join(";")));
}
...
function dueDate(arrivalDate) {
  ...
  const due = new Date(arrivalDate + "T00:00:00");
...
function viewSummary() {
  ...
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      ...
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="14"><div class="emptystate">Нет данных под фильтр. Добавьте загрузку.</div></td></tr>` : rows.map((r, ri) => {
          ...
          <td>${fmtNum(r.shipped)}${(r.ship_trucks || []).length ? ` <button class="btn link sm" title="Фуры, которыми отгружено" data-action="ship-trucks" data-idx="${ri}">…</button>` : ""}</td>
          ...
          <td style="white-space:nowrap"><button class="btn link sm" title="Изменить загрузку" data-action="edit-sum-row" data-idx="${ri}">✎</button><button class="btn link danger sm" title="Удалить загрузку" data-action="del-sum-row" data-idx="${ri}">${TRASH}</button></td>
        </tr>`;
        }).join("")}
      </tbody>
      <tfoot>
    ...
    <p class="sub" style="margin-top:10px">Остаток = пришло − отгружено. Кнопка «…» в колонке «Отгружено» — фуры, которыми отправлено. Иконки ✎ / 🗑 — изменить / удалить загрузку.</p>`;
}
...
// ---------- shipments (отгрузки + оплаты, одна страница) ----------
function viewShipments() {
  ...
  const shown = state.finLogId ? [state.finance.find((g) => Number(g.logistics_id) === Number(state.finLogId))].filter(Boolean) : state.finance;
  return `
    ...
    <div class="card" style="margin-bottom:14px">
      <div style="font-weight:600;margin-bottom:8px">Логисты — нажмите, чтобы показать его отгрузки и оплаты:</div>
      <div style="display:flex;flex-wrap:wrap;gap:8px">
        <button class="btn ${!state.finLogId ? "" : "ghost"}" data-action="fin-log" data-id="">Все логисты</button>
        ${state.logistics.map((l) => `<button class="btn ${Number(state.finLogId) === Number(l.id) ? "" : "ghost"}" data-action="fin-log" data-id="${l.id}">${esc(l.name)}</button>`).join("")}
      </div>
    </div>
    <div class="card" style="margin-bottom:16px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
      <div style="color:var(--muted);font-size:14px">Логистов: <b style="color:var(--fg)">${shown.length}</b> · Фур: <b style="color:var(--fg)">${rows.length}</b></div>
    </div>
    ${sHas ? `<div style="margin-bottom:12px"><button class="btn ghost" data-action="ship-clear">Сбросить фильтры</button></div>` : ""}
    <div class="tbl-wrap"><table class="table">
      <thead>
        <tr><th>Номер документа</th><th>Фура</th><th>Дата отправки</th><th>Дата прибытия</th><th>Логист</th><th>Инвойс (завод)</th><th>Товар</th><th>Куб</th><th>Стоимость</th><th>0.4%</th><th>Итого</th><th>Приём</th><th></th></tr>
        <tr style="background:var(--surface-2)">
          <th>${mfS("doc", "Документ", tDoc)}</th><th>${mfS("truck_no", "Фура", tNo)}</th><th></th><th></th>
          <th>${mfS("logistics", "Логист", tLog)}</th>
          <th>${mfS("pi", "PI", tPi)}</th><th></th><th></th><th></th><th></th>
          <th>${mfS("recv", "Приём", recvOpts)}</th><th></th>
        </tr>
      </thead>
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="12"><div class="emptystate">Отгрузок пока нет.</div></td></tr>` : rows.map((s) => {
          return `
          <tr>
...
function viewFinance() {
  ...
  const finQ = String(state.finQ || "").trim().toLowerCase();
  if (finQ) groups = groups.map((gg) => ({ ...gg, rows: (gg.rows||[]).filter(rr => [rr.doc_number, rr.truck_no, (rr.lines||[]).map(l=>l.model_name||""), (rr.lines||[]).map(l=>l.order_no||"")].join(" ").toLowerCase().includes(finQ)) })).filter(gg => (gg.rows || []).length);
  return `
    <div class="toolbar"><div><h1>Финансы</h1><p class="sub" style="margin:0">Прибывшие грузы · оплата по логисту · срок 30 дней с приёмки.</p></div>
    <div style="display:flex;gap:8px;align-items:center">
      <input id="finQ" class="f-input" placeholder="Фильтр: документ, фура, инвойс, товар…" value="${esc(state.finQ || "")}" style="min-width:230px" />
      <button class="btn ghost" data-action="fin-export">Скачать Excel</button>
      ${state.finQ ? `<button class="btn ghost" data-action="fin-filter-clear">✕</button>` : ""}
    </div><p class="sub" style="margin:0">Прибывшие грузы (после приёмки в «Граница → Ташкент»). Срок оплаты — 30 дней с даты приёмки.</p></div></div>
    <div class="card" style="margin-bottom:14px">
      <div style="font-weight:600;margin-bottom:8px">Логисты — нажмите, чтобы показать его оплаты:</div>
      <div style="display:flex;flex-wrap:wrap;gap:8px">
        <button class="btn ${!state.finLogId ? "" : "ghost"}" data-action="fin-log" data-id="">Все логисты</button>
        ${state.logistics.map((l) => `<button class="btn ${Number(state.finLogId) === Number(l.id) ? "" : "ghost"}" data-action="fin-log" data-id="${l.id}">${esc(l.name)}</button>`).join("")}
      </div>
    </div>
    <div class="card" style="margin-bottom:16px;display:flex;gap:22px;font-size:16px">
      <span>Оплачено: <b style="color:var(--brand)">${fmtNum(totalPaidAll)}</b></span>
      <span>Общий долг: <b style="color:var(--danger)">${fmtNum(totalDebtAll)}</b></span>
    </div>
    <div class="card" style="margin-bottom:16px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px">
      <div>
        <div style="font-weight:600">Оплата нескольких фур разом</div>
        <div style="color:var(--muted);font-size:13px">Отметьте фуры галочками (можно хоть 10 из 100) и нажмите «Оплатить выбранное» — долги спишутся автоматически.</div>
      </div>
      <div style="display:flex;gap:8px;align-items:center">
        <b style="color:var(--brand)">Выбрано: ${selRows.length}</b>
        <span style="color:var(--muted)">долг: ${fmtNum(selDebt)}</span>
        ${selRows.length ? `<button class="btn ghost" data-action="bulk-clear">Сбросить</button><button data-action="bulk-pay">Оплатить выбранное (${selRows.length})</button>` : ""}
      </div>
    </div>
    ${groups.length === 0 ? `<div class="emptystate">Нет прибывших фур. Заполните приёмку в «Граница → Ташкент».</div>` : groups.map((g) => `
      <div class="card" style="margin-bottom:16px">
        ...
        <div class="tbl-wrap"><table class="table" style="font-size:13px">
          <thead><tr><th></th><th>Номер документа</th><th>Фура</th><th>Дата прибытия</th><th>Инвойс (завод)</th><th>Товар</th><th>Итого</th><th>Оплачено</th><th>Долг</th><th>Дней до оплаты</th><th>История оплат</th><th></th></tr></thead>
          <tbody>
            ${g.rows.map((r) => `
              <tr>
...
function openShipTrucks(r) {
  ...
  const overlay = openModal(`
    ...
    <div style="margin-top:12px">${rows.map((t) => `<div style="display:flex;justify-content:space-between;gap:10px;padding:9px 0;border-bottom:1px solid var(--border)"><span><b>${esc(t.truck_no || "—")}</b> · ${fmtDate(t.date)} · Order ${esc(t.order_no || "—")}</span><span style="color:var(--muted)">${fmtNum(t.qty)} шт</span></div>`).join("")}</div>
...
function openShipmentGoods(id) {
  ...
  const csvLines = [["Order №", "Модель", "Кол-во", "Куб"].map(qs).join(";")]
    .concat(rowsArr.map((li) => {
      const m = state.models.find((x) => Number(x.id) === Number(li.model_id));
      const c = (Number(li.qty) || 0) * (m ? (Number(m.cbm_per_pc) || 0) : 0);
      return [li.order_no || "", m ? m.model : li.model_id, li.qty, c.toFixed(2)].map(qs).join(";");
    }));
  const overlay = openModal(`
    ...
    <div class="actions" style="margin-top:12px"><button type="button" class="btn ghost" id="sg-dl">Скачать CSV</button><button type="button" class="btn ghost" data-close>Закрыть</button></div>`, "modal-wide");
  overlay.querySelector("#sg-dl").addEventListener("click", () => downloadCsv("товар_" + (s.truck_no || s.id), csvLines));
  [...overlay.querySelectorAll("[data-close]")].forEach((b) => b.addEventListener("click", () => overlay.remove()));
}
...
function exportSummaryCSV() {
  ...
  rows.forEach((r) => {
    lines.push([r.order_no || "", r.model_name, r.category, r.logistics_name || "", r.factory_name, r.invoice_no, r.received, r.pickup_date, r.shipped, r.qty, (r.qty * (Number(r.cbm_per_pc) || 0)).toFixed(2), r.days_sitting, r.shipment_status].map(csv).join(";"));
  });
  const blob = new Blob(["\uFEFF" + lines.join("\r\n")], { type: "text/csv;charset=utf-8" });
  const a = document.createElement("a");
...
function openBulkPay() {
  ...
  const overlay = openModal(`
    ...
    <div class="card" style="margin:12px 0;padding:10px 14px">
      ${rows.map((r) => `<div style="display:flex;justify-content:space-between;gap:10px;padding:4px 0;border-bottom:1px solid var(--border)"><span>${esc(r.doc_number || "—")} · ${esc(r.truck_no || "—")}</span><span style="color:var(--muted)">долг ${fmtNum(r.debt)}</span></div>`).join("")}
...
function openShipment(item) {
  ...
  function renderLines() {
    ...
    sel.forEach((x, i) => {
      ...
      row.innerHTML = `<b>${esc(x.order_no)}</b> <b style="font-weight:600">${esc(x.model_name)}</b>
        ...
        <span class="over" style="display:none;color:var(--danger);font-size:12px;font-weight:600">больше остатка!</span>
  ...
  function pickGoods() {
    ...
    orders.forEach((o) => {
      ...
      html += `<table class="table" style="font-size:13px"><thead><tr><th></th><th>Модель</th><th>Остаток</th><th>Куб</th><th>Кол-во</th></tr></thead><tbody>`;
      html += items.map((k) => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const st = stock(curLog);
    const sums = {};
    sel.forEach((x) => { if ((Number(x.qty) || 0) > 0) sums[x.ukey] = (sums[x.ukey] || 0) + (Number(x.qty) || 0); });
    const errs = Object.keys(sums).filter((k) => sums[k] > (st[k] || 0)).map((k) => `Order ${k.split("__")[0]} · ${(state.models.find((m) => Number(m.id) === Number(k.split("__")[1])) || {}).model || k.split("__")[1]} (доступно ${st[k]})`);
    if (errs.length) { errBox.textContent = "Нельзя отгрузить больше остатка: " + errs.join("; "); return; }
...
function wire() {
  ...
  (app.querySelectorAll("#finQ") || []).forEach((el) => el.addEventListener("change", () => { state.finQ = el.value; render(); }));
...
async function onClick(e) {
  ...
  switch (action) {
    ...
    case "fin-export": exportFinancesCSV(); break;
    case "fin-filter-clear": state.finQ = ""; render(); break;
    case "ship-trucks": { const r = state.sumRows[Number(id)]; if (r) openShipTrucks(r); } break;
    case "edit-sum-row": { const r = state.sumRows[Number(id)]; if (r) editSumRow(r); } break;
    case "del-sum-row": { const r = state.sumRows[Number(id)]; if (r) delSumRow(r); } break;
    ...
    case "del-shipment": if (confirmBox("Удалить отгрузку? История (платежи) удалится вместе с ней.")) { const r = await api("/api/shipments/" + id, { method: "DELETE" }); r.data.ok ? (toast("Удалено"), await Promise.all([loadAll(), loadFinance()]), render()) : toast(r.data.error, false); } break;
