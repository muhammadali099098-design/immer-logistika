...
function viewSummary() {
  ...
  const tPaid = rows.reduce((a, r) => a + r.cost_amount, 0);
  const showLog = isAdmin && !state.sumFilter;
  return `
    <div class="toolbar"><div><h1>Сводка</h1><p class="sub" style="margin:0">Какой товар у какого логиста, остаток и оплата.</p></div></div>
    ...
    <div class="tbl-wrap"><table class="table">
      <thead><tr><th>Модель</th><th>Категория</th>${showLog ? "<th>Логист</th>" : ""}<th>Остаток</th><th>Объём, м³</th><th>Пришло</th><th>Отгружено</th><th>Завод</th><th>Оплачено</th></tr></thead>
      <tbody>
      ...
      <tfoot>
        <tr style="font-weight:700;background:var(--surface-2)">
          ...
          <td></td><td>${fmtNum(tPaid)}</td>
        </tr>
      </tfoot>
    </table></div>
    <p class="sub" style="margin-top:10px">Остаток = загружено с заводов − отгружено в Узбекистан. Оплачено — сумма за фуры, где этот товар отправлен.</p>`;
}
...
function openTruck() {
  ...
  const logCmd = isAdmin
    ? `<select id="tr_logistic"><option value="">Выберите логиста</option>${state.logistics.map((l) => `<option value="${l.id}">${esc(l.name)}</option>`).join("")}</select>`
    : `<input value="${esc(state.user.company_name || "")}" disabled />`;
  const overlay = openModal(`
    <h3>Записать загрузку (фуру)</h3>
    <form id="truckForm" class="form" style="margin-top:14px">
      ...
      <div class="grid2">
        <div><label>Завод</label><select id="tr_factory"><option value="">Выберите завод</option>${state.factories.map((f) => `<option value="${f.id}">${esc(f.name)}</option>`).join("")}</select></div>
      ...
      <div class="grid2">
        ...
        <div><label>Примечание</label><input id="tr_notes" /></div>
      </div>
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const r = await api("/api/trucks", { method: "POST", body });
    if (r.data.ok) { overlay.remove(); toast("Загрузка записана"); await loadAll(); render(); }
...
async function onClick(e) {
  ...
  switch (action) {
    ...
    case "open-truck": openTruck(); break;
