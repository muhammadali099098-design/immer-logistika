...
function viewSummary() {
  ...
  const mf = (f, label, options) => {
    const sel = arr(f);
    return `<div class="mfilter" data-field="${f}"><button type="button" class="f-input mf-toggle" style="text-align:left">${label}${sel.length ? ` (${sel.length})` : ""} ▾</button>
      <div class="mfilter-pop" hidden>
        <div class="mf-list">${options.map((o) => `<label class="mf-opt"><input type="checkbox" value="${esc(o)}" ${sel.includes(o) ? "checked" : ""}>${esc(o)}</label>`).join("")}</div>
  ...
  const hasF = Object.keys(F).some((k) => (Array.isArray(F[k]) ? F[k].length : F[k]));
  return `
...
function wire() {
  ...
  app.querySelectorAll(".mfilter").forEach((el) => {
    ...
    el.querySelector(".mf-apply").addEventListener("click", () => {
      state.sumFilters[field] = [...el.querySelectorAll(".mf-opt input:checked")].map((c) => c.value);
      render();
    });
    el.querySelector(".mf-clear").addEventListener("click", () => { state.sumFilters[field] = []; render(); });
  });
