    ...
    ${groups.length === 0 ? `<div class="emptystate">Нет прибывших фур. Заполните приёмку в «Граница → Ташкент».</div>` : groups.map((g) => `
      <div class="card" style="margin-bottom:16px">
        ...
        <div class="tbl-wrap"><table class="table" style="font-size:13px">
          <thead><tr><th>Номер документа</th><th>Фура</th><th>Дата прибытия</th><th>Товар</th><th>Итого</th><th>Оплачено</th><th>Долг</th><th>Дней до оплаты</th><th>История оплат</th><th></th></tr></thead>
          <tbody>
            ${g.rows.map((r) => `
              <tr>
...
function openShipTrucks(r) {
  ...
  if (!ts.length) { toast("Загрузок по этой позиции не найдено.", false); return; }
  const overlay = openModal(`
    <h3>Фуры: ${esc(r.model_name)} · инвойс ${esc(r.invoice_no || "—")}</h3>
    <p class="sub" style="margin:6px 0 0">Загрузки, из которых посчитана эта позиция.</p>
    <div style="margin-top:12px">${ts.map((t) => {
      const q = ((t.lines || []).find((l) => Number(l.model_id) === Number(r.model_id))) || {};
      return `<div style="display:flex;justify-content:space-between;gap:10px;padding:9px 0;border-bottom:1px solid var(--border)"><span><b>${esc(t.truck_no || "—")}</b> · ${fmtDate(t.date)} · Order ${esc(t.order_no || "—")}</span><span style="color:var(--muted)">${fmtNum(q.qty || 0)} шт</span></div>`;
    }).join("")}</div>
