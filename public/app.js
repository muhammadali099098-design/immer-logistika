...
async function api(path, opts = {}) {
  ...
  try {
    ...
    if (attempt === 0) { await new Promise((r) => setTimeout(r, 1800)); continue; }
    return { status: 0, data: { ok: false, error: "Сервер не ответил. Хостинг «просыпается» до минуты — запрос уже повторён; если ошибка осталась, нажмите «↻ Обновить»." } };
  }
  try { return { status: res.status, data: await res.json() }; }
  catch { return { status: res.status, data: { ok: false, error: "Нет данных от сервера, попробуйте ещё раз" } }; }
  }
...
function exportFinancesCSV() {
  ...
  const rows = [["Логист", "Номер документа", "Фура", "Дата прибытия", "Срок оплаты", "Дней до оплаты", "Инвойс (завод)", "Товар", "Итого", "Оплачено", "Остаток долга", "Статус"]];
  let tt = 0, tp = 0, td = 0;
  groups.forEach((g) => (g.rows || []).forEach((r) => {
    const iv = invoiceCodes(g.logistics_id, r.lines);
    const d = daysToDue(r.arrival_date);
    tt += Number(r.total) || 0; tp += Number(r.paid) || 0; td += Number(r.debt) || 0;
    rows.push([g.logistics_name, r.doc_number, r.truck_no, r.arrival_date ? fmtDate(r.arrival_date) : "", dueDate(r.arrival_date), d == null ? "" : (d <= 0 ? "ПРОСРОЧЕНО " + Math.abs(d) : d), iv.join(", "), (r.lines || []).map((l) => (l.model_name || "") + " x " + l.qty).join(", "), (Number(r.total) || 0).toFixed(2), (Number(r.paid) || 0).toFixed(2), (Number(r.debt) || 0).toFixed(2), r.debt <= 0 ? "оплачено" : "долг"]);
  }));
  rows.push(["ИТОГО", "", "", "", "", "", "", "", tt.toFixed(2), tp.toFixed(2), td.toFixed(2), ""]);
  downloadCsv("finansy_po_logistam", rows.map((w) => w.map((v) => '"' + String(v == null ? "" : v).replace(/"/g, '""') + '"').join(";")));
}
    ...
    <div class="tbl-wrap"><table class="table">
      ...
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="14"><div class="emptystate">Нет данных под фильтр. Добавьте загрузку.</div></td></tr>` : rows.map((r, ri) => {
          ...
          <td>${(()=>{const st=r.ship_trucks||[];if(!st.length)return fmtNum(r.shipped);const t=v=>`<span style="background:var(--surface-2);border-radius:6px;padding:2px 7px;font-size:12px;color:var(--fg);display:inline-block;margin:2px 4px 2px 0">${esc(v.truck_no||"—")} × ${fmtNum(v.qty)}</span>`;return st.length<=2?st.map(t).join(""):st.slice(0,2).map(t).join("")+` <button type="button" class="btn link sm" style="font-size:12px" data-action="ship-trucks" data-idx="${ri}">все (${st.length})</button>`;})()}</td>
...
function viewFinance() {
  ...
  return `
    <div class="toolbar"><div><h1>Финансы</h1><p class="sub" style="margin:0">Прибывшие грузы (после приёмки в «Граница → Ташкент»). Срок оплаты — 30 дней с даты приёмки.</p></div></div>
    <div class="card" style="margin-bottom:14px">
    ...
    <div class="card" style="margin-bottom:16px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px">
      <div style="display:flex;gap:22px;font-size:16px;align-items:center">
