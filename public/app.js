...
function viewSummary() {
  ...
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      <thead>
        <tr><th>Order №</th><th>Модель</th><th>Категория</th><th>Завод</th><th>Инвойс</th><th>Фура</th><th>Пришло</th><th>Дата забора</th><th>Отгружено</th><th>Остаток</th><th>Общий куб</th><th>Дней</th><th>Статус</th></tr>
...
function viewShipments() {
  ...
  return `
    ...
    <div class="card" style="margin-bottom:16px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
      ...
      <div style="font-size:16px">Всего оплачено: <b style="color:var(--brand)">${fmtNum(totalPaidAll)}</b></div>
    </div>
    ${sHas ? `<div style="margin-bottom:12px"><button class="btn ghost" data-action="ship-clear">Сбросить фильтры</button></div>` : ""}
    <div class="tbl-wrap"><table class="table">
      <thead>
        <tr><th>Номер документа</th><th>Фура</th><th>Дата отправки</th><th>Дата прибытия</th><th>Номер PI</th><th>Товар</th><th>Стоимость</th><th>0.4%</th><th>Итого</th><th>Оплачено</th><th>Долг</th><th>Приём</th><th></th></tr>
        <tr style="background:var(--surface-2)">
          <th>${mfS("doc", "Документ", tDoc)}</th><th>${mfS("truck_no", "Фура", tNo)}</th><th></th><th></th>
          <th>${mfS("pi", "PI", tPi)}</th><th></th><th></th><th></th><th></th><th></th><th></th>
          <th>${mfS("recv", "Приём", recvOpts)}</th><th></th>
        </tr>
      </thead>
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="13"><div class="emptystate">Отгрузок пока нет.</div></td></tr>` : rows.map((s) => {
          ...
          <tr>
            ...
            <td style="white-space:nowrap">
              ...
              ${fr.has_doc ? `<button class="btn link sm" data-action="pay-doc" data-doc="${esc(s.doc_number)}">Оплатить</button>` : `<span style="color:var(--muted);font-size:11px">+№ док ✎</span>`}
...
function openPayDoc(docNumber) {
  const overlay = openModal(`
    <h3>Оплата по документу № ${esc(docNumber)}</h3>
    <form id="payDocForm" class="form" style="margin-top:14px">
      ...
      <div><label>Документ об оплате *</label><input type="file" id="phFile" required /></div>
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!file) { form.querySelector("#phError").textContent = "Загрузите документ"; return; }
    const up = await uploadFile(file);
    if (!up.ok) { form.querySelector("#phError").textContent = up.error || "Ошибка файла"; return; }
    const r = await api("/api/payments", { method: "POST", body: { doc_number: docNumber, amount: Number(form.querySelector("#ph_amount").value), currency: form.querySelector("#ph_currency").value, date: form.querySelector("#ph_date").value, file_name: up.file_name } });
...
function exportSummaryCSV() {
  ...
  const head = ["Order №", "Модель", "Категория", "Завод", "Инвойс", "Фура", "Пришло", "Дата забора", "Отгружено", "Остаток", "Общий куб", "Дней", "Статус"];
  const csv = (v) => `"${String(v == null ? "" : v).replace(/"/g, '""')}"`;
  const lines = [head.map(csv).join(";")];
  rows.forEach((r) => {
    const truckNos = [...new Set((r.load_trucks || []).map((t) => t.truck_no).filter(Boolean))].join(", ");
    lines.push([r.order_no || "", r.model_name, r.category, r.factory_name, r.invoice_no, truckNos, r.received, r.pickup_date, r.shipped, r.qty, (r.qty * (Number(r.cbm_per_pc) || 0)).toFixed(2), r.days_sitting, r.shipment_status].map(csv).join(";"));
  });
...
function openShipment(item) {
  ...
  if (editing && item.lines && item.lines.length) {
    ...
    item.lines.forEach((li) => {
      const uk = key(li.order_no, li.model_id);
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const body = {
      ...
      extra_fee: editing ? form.querySelector("#sh_extra").value : "",
      lines,
    };
...
async function onClick(e) {
  ...
  switch (action) {
    ...
    case "pay-doc": openPayDoc(el.getAttribute("data-doc")); break;
