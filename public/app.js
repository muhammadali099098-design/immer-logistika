...
const state = {
  ...
  sumShowSent: false, // Завод-Граница: показывать «Отправленные позиции» (остаток ≤ 0)
};
...
function viewSummary() {
  ...
  // Отправленные позиции (остаток 0 или отрицательный) — в отдельной вкладке.
  if (state.sumShowSent) rows = rows.filter((r) => Number(r.qty) <= 0);
  else rows = rows.filter((r) => Number(r.qty) > 0);
  ...
  return `
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      ...
      <button class="btn ${state.sumShowSent ? "" : "ghost"}" data-action="sum-sent" title="${state.sumShowSent ? "Показать активные позиции с остатком" : "Показать позиции, которые уже целиком отгружены или списаны"}">${state.sumShowSent ? `Активные позиции (${activeCnt})` : `Отправленные позиции (${sentCnt})`}</button>
    ...
    <div class="tbl-wrap"><table class="table">
      ...
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="14"><div class="emptystate">${state.sumShowSent ? "Отправленных позиций пока нет." : "Нет данных под фильтр. Добавьте загрузку."}</div></td></tr>` : rows.slice().reverse().map((r, ri) => {
...
// received = сколько логист получил с завода по (логист, Order, модель);
// ship = сколько уже отгружено в Ташкент по тому же ключу.
function shipDiffCtx() {
  const recv = {}, ship = {};
  const key = (log, ord, mid) => log + "|" + (ord || "") + "|" + mid;
  (state.trucks || []).forEach((t) => (t.lines || []).forEach((li) => { const k = key(t.logistics_id, t.order_no, li.model_id); recv[k] = (recv[k] || 0) + (Number(li.qty) || 0); }));
  (state.shipments || []).forEach((sh) => (sh.lines || []).forEach((li) => { const k = key(sh.logistics_id, li.order_no, li.model_id); ship[k] = (ship[k] || 0) + (Number(li.qty) || 0); }));
  return { recv, ship, key };
}
// Разница по позиции: загрузили в фуру vs остаток у логиста ДО этой фуры.
//   +N — привезли больше, чем было (напр. 601/600);
//   −N (до −5) — дозабрали последние ≤5 шт., остаток у логиста списывается в 0;
//   иначе — пусто («—»).
function shipDiffLabels(s, ctx) {
  const out = [];
  (s.lines || []).forEach((li) => {
    const k = ctx ? ctx.key(s.logistics_id, li.order_no, li.model_id) : null;
    if (!k) return;
    const q = Number(li.qty) || 0;
    const prior = (ctx.recv[k] || 0) - ((ctx.ship[k] || 0) - q); // остаток до этой фуры
    const d = q - prior;
    let label = null;
    if (d > 0) label = "+" + d;
    else if (d === 0) label = "0";
    else if (d >= -5) label = String(d);
    if (label != null) out.push({ model: li.model_name || String(li.model_id), label });
  });
  return out;
}
...
function viewFinance() {
  ...
  return `
    ...
    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
      ${selRows.length ? `<span style="font-size:13px;color:var(--muted)">Выбрано: <b style="color:var(--fg)">${selRows.length}</b> · долг <b style="color:var(--brand)">${fmtNum(selDebt)}</b></span>` : ""}
      <button class="btn ghost sm" data-action="fin-export">Скачать Excel</button>
      ${selRows.length ? `<button class="btn sm" data-action="bulk-pay">Оплатить выбранное (${selRows.length})</button>` : `<button class="btn ghost sm" disabled title="Сначала отметьте фуры галочками слева от таблицы">Оплата разом</button>`}
    </div></div>
    <div class="card" style="margin-bottom:14px">
    ...
    <div class="fin-bar">
      ...
      ${selRows.length ? `<button class="btn ghost sm" data-action="bulk-clear">Снять выделение</button>` : `<span class="fin-hint">Массовая оплата: отметьте фуры галочками слева, кнопка — сверху справа.</span>`}
    </div>
...
function exportShipmentsXlsx() {
  ...
  rows.forEach((s) => (s.lines || []).forEach((li) => {
    ...
    const k = dctx.key(s.logistics_id, li.order_no, li.model_id);
    const q = Number(li.qty) || 0;
    const prior = (dctx.recv[k] || 0) - ((dctx.ship[k] || 0) - q);
    const d = q - prior;
    let lbl = null;
    if (d > 0) lbl = "+" + d; else if (d === 0) lbl = "0"; else if (d >= -5) lbl = String(d);
...
async function onClick(e) {
  ...
  switch (action) {
    ...
    case "sum-sent": state.sumShowSent = !state.sumShowSent; render(); break;
