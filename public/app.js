...
function viewSummary() {
  ...
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      <thead>
        <tr><th>Order №</th><th>Модель</th><th>Категория</th><th>Логист</th><th>Завод</th><th>Инвойс</th><th>Фура</th><th>Пришло</th><th>Дата забора</th><th>Отгружено</th><th>Остаток</th><th>Общий куб</th><th>Дней</th><th>Статус</th></tr>
        ...
        ${rows.length === 0 ? `<tr><td colspan="14"><div class="emptystate">Отгрузок пока нет.</div></td></tr>` : rows.map((s) => {
          ...
          <tr>
            ...
            <td style="white-space:nowrap">
              ...
              <button class="btn link sm" data-action="pay-doc" data-doc="${esc(s.doc_number || "")}" data-id="${s.id}">Оплатить</button>
...
function exportSummaryCSV() {
  ...
  const head = ["Order №", "Модель", "Категория", "Логист", "Завод", "Инвойс", "Фура", "Пришло", "Дата забора", "Отгружено", "Остаток", "Общий куб", "Дней", "Статус"];
  const csv = (v) => `"${String(v == null ? "" : v).replace(/"/g, '""')}"`;
  const lines = [head.map(csv).join(";")];
  rows.forEach((r) => {
    const truckNos = [...new Set((r.load_trucks || []).map((t) => t.truck_no).filter(Boolean))].join(", ");
    lines.push([r.order_no || "", r.model_name, r.category, r.logistics_name || "", r.factory_name, r.invoice_no, truckNos, r.received, r.pickup_date, r.shipped, r.qty, (r.qty * (Number(r.cbm_per_pc) || 0)).toFixed(2), r.days_sitting, r.shipment_status].map(csv).join(";"));
  });
