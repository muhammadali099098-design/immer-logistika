...
function exportFinancesCSV() {
  ...
  downloadCsv("finansy_po_logistam", rows.slice().reverse().map((w) => w.map((v) => '"' + String(v == null ? "" : v).replace(/"/g, '""') + '"').join(";")));
}
...
function viewSummary() {
  ...
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      ...
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="14"><div class="emptystate">Нет данных под фильтр. Добавьте загрузку.</div></td></tr>` : rows.slice().reverse().map((r, ri) => {
...
function viewShipments() {
  ...
  return `
    ...
    <div class="tbl-wrap"><table class="table">
      ...
      <tbody>
        ${rows.length === 0 ? `<tr><td colspan="12"><div class="emptystate">Отгрузок пока нет.</div></td></tr>` : rows.slice().reverse().map((s) => {
          return `
          <tr>
