...
function shipDiffCtx() {
  const recv = {};
  const key = (log, mid) => log + "|" + mid;
  (state.trucks || []).forEach((t) => (t.lines || []).forEach((li) => { const k = key(t.logistics_id, li.model_id); recv[k] = (recv[k] || 0) + (Number(li.qty) || 0); }));
  const groups = {};
  (state.shipments || []).forEach((sh) => (sh.lines || []).forEach((li) => {
    const k = key(sh.logistics_id, li.model_id);
  ...
  Object.keys(groups).forEach((k) => {
    const list = groups[k].sort((a, b) => a.date < b.date ? -1 : a.date > b.date ? 1 : (Number(a.sid) - Number(b.sid)));
    ...
    list.forEach((g) => {
      ...
      let label = null;
      if (cum > recvT) {
        if (!crossed) { crossed = true; label = "+" + (cum - recvT); } // фура, из-за которой сумма превысила заводскую
      } else {
        const remAfter = recvT - cum;
        if (remAfter >= 1 && remAfter <= 5) label = String(-remAfter);
      }
      if (label != null) (byShip[g.sid] = byShip[g.sid] || []).push({ mid: g.mid, model: g.name, label });
    });
  });
  return { recv, key, byShip };
}
