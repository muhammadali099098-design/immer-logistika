...
function shipDiffCtx() {
  ...
  Object.keys(groups).forEach((k) => {
    ...
    list.forEach((g, idx) => {
      ...
      let label = null;
      if (cum > recvT) {
        // «+N» — только на фуре, которая первой сделала сумму больше полученного по этому ордеру
        if (!crossed) { crossed = true; label = "+" + (cum - recvT); }
      } else if (isLast) {
        // «−N» метка — только у ПОСЛЕДНЕЙ фуры ордера (она либо оставила открытым остаток 1..5,
        // либо забрала его ровно до нуля). Промежуточные фуры метки не получают.
        const remAfter = recvT - cum;
        const remBefore = remAfter + g.qty;
        let show = 0;
        if (remAfter > 0 && remAfter <= 5) show = remAfter;
