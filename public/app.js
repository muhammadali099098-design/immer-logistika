...
function openShipment(item) {
  ...
  const overlay = openModal(`
    ...
    <form id="shipmentForm" class="form" style="margin-top:14px">
      ...
      <div class="grid2">
        <div><label>Order № *</label><select id="sh_order"><option value="">Выберите Order №</option></select></div>
      ...
      <div><label>Товар (из выбранного Order №)</label><div id="sh_lines"></div>
        <button type="button" class="btn ghost sm" data-line-add>+ Добавить модель</button></div>
  ...
  const lineArr = [];
  function availForOrder(logId, orderNo) {
    const loaded = {}, shipped = {};
    state.trucks.filter((t) => Number(t.logistics_id) === Number(logId) && String(t.order_no) === String(orderNo)).forEach((t) => {
      (t.lines || []).forEach((li) => { loaded[li.model_id] = (loaded[li.model_id] || 0) + (Number(li.qty) || 0); });
    });
    state.shipments.filter((s) => Number(s.logistics_id) === Number(logId)).forEach((s) => {
      (s.lines || []).forEach((li) => { if (String(li.order_no) === String(orderNo)) shipped[li.model_id] = (shipped[li.model_id] || 0) + (Number(li.qty) || 0); });
    });
    return state.models
      .filter((m) => ((Number(loaded[m.id]) || 0) - (Number(shipped[m.id]) || 0)) > 0)
      .map((m) => ({ id: m.id, name: m.model, qty: (Number(loaded[m.id]) || 0) - (Number(shipped[m.id]) || 0), cbm_per_pc: m.cbm_per_pc }));
  }
  const shLogSel = form.querySelector("#sh_logistic");
  const orderSel = form.querySelector("#sh_order");
  let currentOrder = "";
  if (isAdmin && shLogSel && !shLogSel.value && state.logistics.length > 0) {
    shLogSel.value = String(state.logistics[0].id);
  }
  const initLog = editing ? Number(item.logistics_id) : (isAdmin ? (shLogSel ? Number(shLogSel.value) : "") : Number(state.user.company_id));
  function fillOrderSelect(logId, selOrder) {
    const opts = [...new Set(state.trucks.filter((t) => !logId || Number(t.logistics_id) === Number(logId)).map((t) => String(t.order_no || "").trim()).filter(Boolean))].sort();
    orderSel.innerHTML = `<option value="">Выберите Order №</option>` + opts.map((o) => `<option value="${esc(o)}" ${selOrder && o === selOrder ? "selected" : ""}>${esc(o)}</option>`).join("");
  }
  function renderOrderArea(logId, orderNo, prefill) {
    currentOrder = orderNo || "";
    const na = currentOrder ? availForOrder(logId, currentOrder) : [];
    linesBox.innerHTML = "";
    lineArr.length = 0;
    if (!currentOrder) {
      linesBox.innerHTML = `<div class="emptystate" style="padding:10px">Выберите Order №, чтобы увидеть товары этого заказа.</div>`;
    } else if (prefill && prefill.length) {
      prefill.forEach((li) => {
        if (!na.some((a) => Number(a.id) === Number(li.model_id))) {
          const m = state.models.find((x) => Number(x.id) === Number(li.model_id));
          if (m) na.push({ id: m.id, name: m.model, qty: 0, cbm_per_pc: m.cbm_per_pc });
        }
        addLine({ model_id: li.model_id, qty: li.qty }, na);
      });
    } else if (na.length === 0) {
      linesBox.innerHTML = `<div class="emptystate" style="padding:10px">У этого заказа нет доступных товаров. Сначала добавьте загрузку по этому Order №.</div>`;
    } else {
      addLine(null, na);
    }
  }
  function setupOrder(logId) {
    fillOrderSelect(logId, "");
    if (orderSel.value) renderOrderArea(logId, orderSel.value, null);
    else renderOrderArea(logId, null, null);
  }
  if (shLogSel) shLogSel.addEventListener("change", () => setupOrder(Number(shLogSel.value)));
  orderSel.addEventListener("change", () => {
    const lid = initLog || (shLogSel ? Number(shLogSel.value) : Number(state.user.company_id));
    renderOrderArea(lid, orderSel.value, null);
  });
  function addLine(pre, availList) {
    const row = document.createElement("div");
    row.style.cssText = "display:flex;gap:8px;margin-bottom:8px;align-items:center";
    const list = availList || [];
    row.innerHTML = `<select class="line-model" style="flex:1.4"><option value="">Модель</option>${list.map((x) => `<option value="${x.id}" ${pre && Number(pre.model_id) === Number(x.id) ? "selected" : ""}>${esc(x.name)} (в нал. ${fmtNum(x.qty)})</option>`).join("")}</select>
      <input class="line-qty" type="number" min="0" step="any" value="${pre ? pre.qty : ""}" placeholder="Кол-во" style="flex:1" />
      <span class="line-cbm" style="min-width:80px;color:var(--muted);font-size:12px">—</span>
      <button type="button" class="btn danger sm" data-line-del>×</button>`;
    const mSel = row.querySelector(".line-model");
  ...
  lineArr.length = 0;
  if (initLog) {
    const orders = fillOrderSelect(initLog, editing && item.lines && item.lines[0] ? item.lines[0].order_no || "" : "");
    if (orderSel.value) renderOrderArea(initLog, orderSel.value, editing ? item.lines || [] : null);
    else renderOrderArea(initLog, null, null);
  } else {
    linesBox.innerHTML = `<div class="emptystate" style="padding:10px">Выберите логиста и Order №.</div>`;
  }
  form.querySelector("[data-line-add]").addEventListener("click", () => {
    const lid = initLog || (shLogSel ? Number(shLogSel.value) : Number(state.user.company_id));
    const na = currentOrder ? availForOrder(lid, currentOrder) : [];
    if (currentOrder && na.length) addLine(null, na);
  });
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    const lines = lineArr.map((r) => ({ model_id: Number(r.querySelector(".line-model").value), qty: Number(r.querySelector(".line-qty").value), order_no: currentOrder })).filter((l) => l.model_id && l.qty > 0);
    const body = {
