...
function viewFinance() {
  ...
  return `
    ...
    <div class="fin-bar">
      <span class="fin-stat">Оплачено: <b style="color:var(--brand)">${fmtNum(totalPaidAll)}</b></span>
      <span class="fin-stat">Общий долг: <b style="color:var(--danger)">${fmtNum(totalDebtAll)}</b></span>
      <span class="fin-stat" style="color:var(--muted)">Фур: <b style="color:var(--fg)">${groups.reduce((a, g) => a + (g.rows || []).length, 0)}</b></span>
      ${finF.doc || finF.truck || finF.inv ? `<button class="btn ghost sm" data-action="fin-filter-clear">Сбросить фильтры</button>` : ""}
      ${selRows.length
        ? `<span class="fin-stat">Выбрано: <b>${selRows.length}</b> · долг: <b style="color:var(--brand)">${fmtNum(selDebt)}</b></span>
           <button class="btn sm" data-action="bulk-pay">Оплатить выбранное</button>
           <button class="btn ghost sm" data-action="bulk-clear">Сбросить</button>`
        : `<span class="fin-hint">Массовая оплата: отметьте фуры галочками слева — кнопка появится здесь.</span>`}
